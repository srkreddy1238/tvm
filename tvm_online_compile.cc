...
int main() {
  ...
  tvm::ffi::Array<tvm::PrimExpr> tvm_shape_c = {1, 3, 4, 5};
  ...
  auto input_tensor_c = tvm::runtime::Tensor::Empty(tvm::ffi::Shape({1, 3, 4, 5}), dl_type,
                                                    tvm::Device({dl_dev_type, 0}), std::nullopt);

  auto C = tvm::relax::PrimValue(tvm::PrimExpr(3.0f));
  // auto C = tvm::relax::Var("y", tvm::relax::TensorStructInfo(dtype, 4));
  // //tvm::relax::PrimValue(); VarNode
  ...
  */
  ...
  auto input_tensor_a = tvm::runtime::Tensor::Empty(tvm::ffi::Shape({2, 3, 4, 5}), dl_type,
                                                    tvm::Device({dl_dev_type, 0}), std::nullopt);
  auto input_tensor_b = tvm::runtime::Tensor::Empty(tvm::ffi::Shape({2, 3, 4, 5}), dl_type,
                                                    tvm::Device({dl_dev_type, 0}), std::nullopt);
