...
# under the License.
"""Test for _attention_sequence_prefill - MHA h_kv=8, h_q=8, d=64, float32, non-causal."""
...
class TestAttentionSequencePrefillGPU_MHA_F32_NonCausal:
    @pytest.fixture(autouse=True)
    def _target(self):
        self.target = tvm.target.Target("cuda")

    def test_mha_f32_non_causal(self):
        fn = _attention_sequence_prefill(8, 8, 64, "float32", self.target, causal=0)
        ...
        def batch_sequence_prefill_kv(var_q: T.handle, var_k: T.handle, var_v: T.handle, var_output: T.handle, var_lse: T.handle):
            ...
            q = T.match_buffer(var_q, (batch_size, qo_len, 8, 64))
            kv_len = T.int32(is_size_var=True)
            k = T.match_buffer(var_k, (batch_size, kv_len, 8, 64))
            v = T.match_buffer(var_v, (batch_size, kv_len, 8, 64))
            output = T.match_buffer(var_output, (batch_size, qo_len, 8, 64))
            lse = T.match_buffer(var_lse, (batch_size, qo_len, 8))
            # with T.sblock("root"):
            batch_tiles: T.int32 = (qo_len + 16 - 1) // 16
                    ...
                    for lty in T.thread_binding(4, thread="threadIdx.y"):
                        for ltx in T.thread_binding(32, thread="threadIdx.x"):
                            with T.sblock("attn"):
                                ...
                                K_smem = T.alloc_buffer((16, 64), scope="shared")
                                ...
                                T.tvm_storage_sync("shared")
                                for iterator in range((kv_len + 15) // 16):
                                    L_kv_start: T.int32 = iterator * 16
                                    L_kv_base: T.int32 = 0
                                    ...
                                    T.tvm_storage_sync("shared")
                                    with T.sblock(""):
                                        T.reads(Q_smem[0:16, 0:64], K_smem[0:16, 0:64])
                                                            ...
                                                            for lj_1_1 in T.vectorized(2):
                                                                for lk_1 in range(16):
                                                                    with T.sblock("S_gemm_update"):
                                                                        ...
                                                                        T.writes(S_local[i, j])
                                                                        S_local[i, j] = S_local[i, j] + Q_smem[i, k_1] * K_smem[j, k_1] * T.float32(1.4426950408889634)
                                    T.tvm_storage_sync("shared")
                                    for li_0_lj_0_fused_0 in T.thread_binding(4, thread="threadIdx.y"):
                                        for li_0_lj_0_fused_1 in T.thread_binding(32, thread="threadIdx.x"):
                                            for li_1 in range(1):
                                    ...
                                    for i in range(1):
                                        ...
                                        if row < 16:
                                            with T.sblock("update1"):
                                                T.reads(m_smem[row], m_new[i], S_smem[row, 0:16], d_smem[row], m_prev[i])
                                                T.writes(m_prev[i], m_new[i], d_new[i])
                                                m_prev[i] = m_smem[row]
                                                m_new[i] = m_smem[row]
                                                _ = LH_start + row
                                                for j in range(16):
                                                    if L_kv_start + j < kv_len:
                                                        m_new[i] = T.max(m_new[i], S_smem[row, j])
                                                d_new[i] = d_smem[row] * T.exp2(m_prev[i] - m_new[i])
                                    for i in range(1):
                                        row: T.int32 = i * 32 * 4 + ty * 32 + tx
                                        with T.sblock("update"):
                                            T.reads(S_smem[row, 0:16], m_new[i])
                                            ...
                                            for lj_1_0 in T.unroll(2):
                                                for lj_1_1 in T.vectorized(2):
                                                    with T.sblock("O_store"):
                                                        ...
                                                        T.writes(output[b_idx, LH_start + i, by, j])
                                                        cur_L: T.int32 = LH_start + i
                                                        cur_H_qo: T.int32 = by
                                                        if cur_L < qo_len:
                                    ...
                                    for li_1 in T.thread_binding(4, thread="threadIdx.y"):
                                        for li_2 in T.thread_binding(32, thread="threadIdx.x"):
                                            with T.sblock("lse_store"):
                                                ...
                                                T.writes(lse[b_idx, LH_start + i, by])
                                                cur_L: T.int32 = LH_start + i
                                                cur_H_qo: T.int32 = by
                                                if cur_L < qo_len:
