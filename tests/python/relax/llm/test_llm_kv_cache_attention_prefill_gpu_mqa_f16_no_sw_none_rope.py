...
# under the License.
"""Test for _attention_prefill - MQA h_kv=1, h_q=4, d=128, float16, no sliding window, no rope."""
...
class TestAttentionPrefillGPU_MQA_F16_NoSW_NoneRope:
    @pytest.fixture(autouse=True)
    def _target(self):
        self.target = tvm.target.Target("cuda")

    def test_mqa_f16_no_sw_none_rope(self):
        fn = _attention_prefill(1, 4, 128, "float16", False, ROPE_SCALING_NONE, self.target)
        ...
        def batch_prefill_paged_kv(var_q: T.handle, var_q_indptr: T.handle, var_pages: T.handle, var_page_indptr: T.handle, var_page_values: T.handle, var_length_info: T.handle, var_k_rope_pos_offset: T.handle, var_q_rope_position: T.handle, var_output: T.handle, var_lse: T.handle, causal: T.int32, rotary_mode: T.int32, rope_scale: T.float32, rope_theta: T.float32, sm_scale: T.float32):
            ...
            q = T.match_buffer(var_q, (total_len, 4, 128), "float16")
            q_indptr = T.match_buffer(var_q_indptr, (batch_size + 1,), "int32", elem_offset=q_indptr_elem_offset)
            pages = T.match_buffer(var_pages, (max_num_pages, 2, 1, 16, 128), "float16", elem_offset=pages_elem_offset)
            ...
            output = T.match_buffer(var_output, (total_len, 4, 128), "float16")
