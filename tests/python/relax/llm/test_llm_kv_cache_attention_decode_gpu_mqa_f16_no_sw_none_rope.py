...
# under the License.
"""Test for _attention_decode - MQA h_kv=1, h_q=4, d=128, float16, no sliding window, no rope."""
...
class TestAttentionDecodeGPU_MQA_F16_NoSW_NoneRope:
    @pytest.fixture(autouse=True)
    def _target(self):
        self.target = tvm.target.Target("cuda")

    def test_mqa_f16_no_sw_none_rope(self):
        fn = _attention_decode(1, 4, 128, "float16", False, ROPE_SCALING_NONE, self.target)
        ...
        def batch_decode_paged_kv(Q_handle: T.handle, pages_handle: T.handle, page_table_indptr_handle: T.handle, page_table_values_handle: T.handle, var_length_info: T.handle, k_rope_pos_offset_handle: T.handle, q_rope_position_handle: T.handle, output_handle: T.handle, lse_handle: T.handle, rotary_mode: T.int32, rope_scale: T.float32, rope_theta: T.float32, sm_scale: T.float32):
            ...
            Q = T.match_buffer(Q_handle, (B, 4, 128), "float16")
            pages = T.match_buffer(pages_handle, (max_num_pages, 2, 1, 16, 128), "float16", elem_offset=pages_elem_offset)
            ...
            output = T.match_buffer(output_handle, (B, 4, 128), "float16")
            lse = T.match_buffer(lse_handle, (B, 4))
            # with T.sblock("root"):
            for bx in T.thread_binding(B, thread="blockIdx.x"):
                for fused_by_bz in T.thread_binding(1, thread="blockIdx.y"):
                    for ty in T.thread_binding(4, thread="threadIdx.y"):
                        for tx in T.thread_binding(32, thread="threadIdx.x"):
                            for tz in T.thread_binding(4, thread="threadIdx.z"):
                                with T.sblock("attn"):
                                    T.reads(page_table_indptr[bx:bx + 2], length_info[bx], q_rope_position[bx], Q[bx, fused_by_bz * 4 + ty, tx * 4 - 64:tx * 4 - 64 + 132])
                                    T.writes(output[bx, T.Mul(0, 4) + fused_by_bz * 4 + ty, tx * 4:tx * 4 + 4], lse[bx, T.Mul(0, 4) + fused_by_bz * 4 + ty])
                                    ...
                                    for iterator in range((kv_chunk_len[0] + 15) // 16):
                                        tile_start_s: T.int32(is_size_var=True) = tz * 4 + ty
                                        tile_start_g: T.int32(is_size_var=True) = (iterator * 4 + tz) * 4 + ty
                                        for j in range(1):
                                        ...
                                        for j in range(4):
                                            for vec in T.vectorized(4):
                                                QK_local[vec] = T.Cast("float32", Q_local[vec]) * T.Cast("float32", K_smem[tz * 4 + j, tx * 4 + vec]) * sm_scale * T.float32(1.4426950408889634)
