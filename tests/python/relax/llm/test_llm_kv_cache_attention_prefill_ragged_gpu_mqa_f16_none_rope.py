...
# under the License.
"""Test for _attention_prefill_ragged - MQA h_kv=1, h_q=4, d_qk=128, d_v=128, float16, no rope."""
...
class TestAttentionPrefillRaggedGPU_MQA_F16_NoneRope:
    @pytest.fixture(autouse=True)
    def _target(self):
        self.target = tvm.target.Target("cuda")

    def test_mqa_f16_none_rope(self):
        fn = _attention_prefill_ragged(1, 4, 128, 128, "float16", ROPE_SCALING_NONE, self.target)
        ...
        def batch_prefill_ragged_kv(var_q: T.handle, var_q_indptr: T.handle, var_k: T.handle, var_v: T.handle, var_kv_indptr: T.handle, var_q_rope_position: T.handle, var_k_rope_pos_offset: T.handle, var_output: T.handle, var_lse: T.handle, causal: T.int32, rotary_mode: T.int32, rope_scale: T.float32, rope_theta: T.float32, sm_scale: T.float32):
            ...
            q = T.match_buffer(var_q, (qo_len, 4, 128), "float16")
            q_indptr = T.match_buffer(var_q_indptr, (batch_size + 1,), "int32", elem_offset=q_indptr_elem_offset)
            k = T.match_buffer(var_k, (kv_len, 1, 128), "float16")
            v = T.match_buffer(var_v, (kv_len, 1, 128), "float16")
            ...
            output = T.match_buffer(var_output, (qo_len, 4, 128), "float16")
