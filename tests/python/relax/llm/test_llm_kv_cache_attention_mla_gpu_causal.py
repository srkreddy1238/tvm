...
# under the License.
"""Test for _attention_prefill_mla - num_heads=32, v_head_dim=128, qk_nope_head_dim=64, float16, causal."""
...
class TestAttentionPrefillMlaGPU_Causal:
    @pytest.fixture(autouse=True)
    def _target(self):
        self.target = tvm.target.Target("cuda")

    def test_causal(self):
        fn = _attention_prefill_mla(32, 128, 64, "float16", True, self.target)

        # fmt: off
        # from tvm.script import tir as T

        @T.prim_func
        def batch_prefill_paged_kv_mla_sliding_window(var_q: T.handle, var_q_indptr: T.handle, var_pages: T.handle, var_page_indptr: T.handle, var_page_values: T.handle, var_length_info: T.handle, var_output: T.handle, var_lse: T.handle, causal: T.int32, sm_scale: T.float32):
            T.func_attr({"tir.is_scheduled": True, "global_symbol": "batch_prefill_paged_kv_mla_sliding_window"})
            ...
            q = T.match_buffer(var_q, (total_len, 32, 192), "float16")
            q_indptr = T.match_buffer(var_q_indptr, (batch_size + 1,), "int32", elem_offset=q_indptr_elem_offset)
            pages = T.match_buffer(var_pages, (max_num_pages, 16, 192), "float16", elem_offset=pages_elem_offset)
            ...
            length_info = T.match_buffer(var_length_info, (3, batch_size), "int32", elem_offset=length_info_elem_offset)
                ...
                for lty in T.thread_binding(4, thread="threadIdx.y"):
                    for ltx in T.thread_binding(32, thread="threadIdx.x"):
                        with T.sblock("attn"):
                            ...
                            Q_smem = T.alloc_buffer((32, 192), "float16", scope="shared")
                            KV_smem = T.alloc_buffer((32, 192), "float16", scope="shared")
                            ...
                            tile_id[0] = bx
                            batch_idx[0] = 0
                            batch_rows[0] = (q_indptr[1] - q_indptr[0]) * 32
                            ...
                            while T.tvm_thread_invariant(batch_idx[0] < batch_size):
                                while tile_id[0] >= batch_tiles[0] and batch_idx[0] < batch_size:
                                    ...
                                    if batch_idx[0] < batch_size:
                                        ...
                                        batch_rows[0] = (q_indptr[b_idx + 1] - q_indptr[b_idx]) * 32
                                        batch_tiles[0] = (batch_rows[0] + 32 - 1) // 32
                                if T.tvm_thread_invariant(batch_idx[0] < batch_size):
                                    ...
                                    for li_1_lj_0_1_fused_0 in T.thread_binding(4, thread="threadIdx.y"):
                                        for li_1_lj_0_1_fused_1 in T.thread_binding(32, thread="threadIdx.x"):
                                            for li_0, lj_0_0 in T.grid(2, 6):
                                                for lj_1 in T.vectorized(4):
                                                    with T.sblock("Q_load"):
                                                        i = T.axis.spatial(32, li_0 * 16 + (li_1_lj_0_1_fused_0 * 32 + li_1_lj_0_1_fused_1) // 8)
                                                        j = T.axis.spatial(192, lj_0_0 * 32 + (li_1_lj_0_1_fused_0 * 32 + li_1_lj_0_1_fused_1) % 8 * 4 + lj_1)
                                                        T.reads()
                                                        T.writes()
                                                        cur_L: T.int32 = q_indptr_val + (LH_start + i) // 32
                                                        cur_H_qo: T.int32 = (LH_start + i) % 32
                                    ...
                                    for iterator_1 in range((kv_chunk_len[0] + 31) // 32):
                                        ...
                                        for lz_1_ly_0_1_fused_0 in T.thread_binding(4, thread="threadIdx.y"):
                                            for lz_1_ly_0_1_fused_1 in T.thread_binding(32, thread="threadIdx.x"):
                                                for lz_0, ly_0_0 in T.grid(2, 6):
                                                    for ly_1 in T.vectorized(4):
                                                        with T.sblock("KV_load"):
                                                            i = T.axis.spatial(32, lz_0 * 16 + (lz_1_ly_0_1_fused_0 * 32 + lz_1_ly_0_1_fused_1) // 8)
                                                            j = T.axis.spatial(192, ly_0_0 * 32 + (lz_1_ly_0_1_fused_0 * 32 + lz_1_ly_0_1_fused_1) % 8 * 4 + ly_1)
                                                            T.reads()
                                                            T.writes()
                                                            cur_L: T.int32 = L_kv_start + i
                                                            if cur_L < kv_chunk_len[0]:
                                                                seq_offset: T.int32(is_size_var=True) = T.if_then_else(cur_L < length_info[2, b_idx], cur_L, cur_L - length_info[2, b_idx] + length_info[1, b_idx])
                                        ...
                                        T.tvm_storage_sync("shared")
                                        with T.sblock(""):
                                            T.reads(Q_smem[0:32, 0:192], KV_smem[0:32, 0:192])
                                                                ...
                                                                for lj_1_1 in T.vectorized(4):
                                                                    for lk_1 in range(16):
                                                                        with T.sblock("S_gemm_update"):
                                                                            ...
                                                                            k = T.axis.reduce(192, lk_0 * 16 + lk_1)
                                        ...
                                        T.tvm_storage_sync("shared")
                                        with T.sblock(""):
                                            T.reads(m_prev_smem[0:32], m_smem[0:32], S_smem[0:32, 0:32], KV_smem[0:32, 0:128])
                                                ...
                                                for lj_1_0 in T.unroll(2):
                                                    for lj_1_1 in T.vectorized(4):
                                                        with T.sblock("O_store"):
                                                            ...
                                                            T.writes(output[q_indptr[b_idx] + (LH_start + i) // 32, (LH_start + i) % 32, j])
                                                            cur_L: T.int32 = q_indptr[b_idx] + (LH_start + i) // 32
                                                            cur_H_qo: T.int32 = (LH_start + i) % 32
                                                            if cur_L < q_indptr[b_idx + 1]:
                                        ...
                                        for li_1 in T.thread_binding(4, thread="threadIdx.y"):
                                            for li_2 in T.thread_binding(32, thread="threadIdx.x"):
                                                with T.sblock("lse_store"):
                                                    ...
                                                    T.writes(lse[q_indptr[b_idx] + (LH_start + i) // 32, (LH_start + i) % 32])
                                                    cur_L: T.int32 = q_indptr[b_idx] + (LH_start + i) // 32
                                                    cur_H_qo: T.int32 = (LH_start + i) % 32
                                                    if cur_L < q_indptr[b_idx + 1]:
        ...
        # fmt: on
        tvm.ir.assert_structural_equal(fn, batch_prefill_paged_kv_mla_sliding_window)
