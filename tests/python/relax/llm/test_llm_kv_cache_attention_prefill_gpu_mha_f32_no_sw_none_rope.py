...
# under the License.
"""Test for _attention_prefill - MHA h_kv=8, h_q=8, d=64, float32, no sliding window, no rope."""
...
class TestAttentionPrefillGPU_MHA_F32_NoSW_NoneRope:
    @pytest.fixture(autouse=True)
    def _target(self):
        self.target = tvm.target.Target("cuda")

    def test_mha_f32_no_sw_none_rope(self):
        fn = _attention_prefill(8, 8, 64, "float32", False, ROPE_SCALING_NONE, self.target)
        ...
        def batch_prefill_paged_kv(var_q: T.handle, var_q_indptr: T.handle, var_pages: T.handle, var_page_indptr: T.handle, var_page_values: T.handle, var_length_info: T.handle, var_k_rope_pos_offset: T.handle, var_q_rope_position: T.handle, var_output: T.handle, var_lse: T.handle, causal: T.int32, rotary_mode: T.int32, rope_scale: T.float32, rope_theta: T.float32, sm_scale: T.float32):
            ...
            pages = T.match_buffer(var_pages, (max_num_pages, 2, 8, 16, 64), elem_offset=pages_elem_offset)
                                                        ...
                                                        for ly_1 in T.vectorized(2):
                                                            with T.sblock("K_load"):
                                                                ...
                                                                if cur_L < kv_chunk_len[0]:
                                                                    ...
                                                                    freq = T.float32()
                                                                    K_smem[i, j] = T.if_then_else(rotary_mode == 1, T.Let(T.cos(freq) * pages[page_no, 0, by, page_offset, j] + T.sin(freq) * T.if_then_else(j < 32, pages[page_no, 0, by, page_offset, j + 32] * T.float32(-1.0), pages[page_no, 0, by, page_offset, j - 32]), where={freq: T.Cast("float32", k_rope_pos_offset[b_idx] + cur_L) * rope_scale / T.pow(rope_theta, T.Cast("float32", j * 2 % 64) / T.float32(64.0))}), pages[page_no, 0, by, page_offset, j])
                                                                else:
                                                                    ...
                                                                    for lj_1_1 in T.vectorized(2):
                                                                        for lk_1 in range(16):
                                                                            with T.sblock("S_gemm_update"):
                                                                                ...
                                                                                k = T.axis.reduce(64, lk_0 * 16 + lk_1)
                                                                                T.reads(S_local[i, j], Q_smem[i, k], K_smem[j, k])
                                                                                T.writes(S_local[i, j])
                                                                                S_local[i, j] = S_local[i, j] + Q_smem[i, k] * K_smem[j, k] * sm_scale * T.float32(1.4426950408889634)
                                            T.tvm_storage_sync("shared")
                                            for li_0_lj_0_fused_0 in T.thread_binding(4, thread="threadIdx.y"):
                                                for li_0_lj_0_fused_1 in T.thread_binding(32, thread="threadIdx.x"):
                                                    for li_1 in range(1):
                                                                ...
                                                                for lj_1_0 in T.unroll(2):
                                                                    for lj_1_1 in T.vectorized(2):
                                                                        with T.sblock("O_gemm_update"):
                                                                            ...
                                                                            T.writes(O_local[i, j])
                                                                            O_local[i, j] = O_local[i, j] + S_smem[i, k] * V_smem[k, j]
                                        for li_0_lj_0_fused_0 in T.thread_binding(4, thread="threadIdx.y"):
                                            for li_0_lj_0_fused_1 in T.thread_binding(32, thread="threadIdx.x"):
                                                for li_1 in range(2):
                                                    for lj_1_0 in T.unroll(2):
