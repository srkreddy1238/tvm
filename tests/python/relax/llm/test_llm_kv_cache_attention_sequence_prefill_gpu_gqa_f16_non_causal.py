...
# under the License.
"""Test for _attention_sequence_prefill - GQA h_kv=4, h_q=32, d=128, float16, non-causal."""
...
class TestAttentionSequencePrefillGPU_GQA_F16_NonCausal:
    @pytest.fixture(autouse=True)
    def _target(self):
        self.target = tvm.target.Target("cuda")

    def test_gqa_f16_non_causal(self):
        fn = _attention_sequence_prefill(4, 32, 128, "float16", self.target, causal=0)
                    ...
                    for lty in T.thread_binding(4, thread="threadIdx.y"):
                        for ltx in T.thread_binding(32, thread="threadIdx.x"):
                            with T.sblock("attn"):
                                ...
                                T.tvm_storage_sync("shared")
                                for iterator_1 in range((kv_len + 31) // 32):
                                                            ...
                                                            for lj_1_1 in T.vectorized(4):
                                                                for lk_1 in range(16):
                                                                    with T.sblock("S_gemm_update"):
                                                                        ...
                                                                        k_ax = T.axis.reduce(128, lk_0 * 16 + lk_1)
                                                                        T.reads(S_local[i, j], Q_smem[i, k_ax], K_smem[j, k_ax])
                                                                        T.writes(S_local[i, j])
                                                                        S_local[i, j] = S_local[i, j] + T.Cast("float32", Q_smem[i, k_ax]) * T.Cast("float32", K_smem[j, k_ax]) * T.float32(1.4426950408889634)
                                    T.tvm_storage_sync("shared")
                                    for li_0_lj_0_fused_0 in T.thread_binding(4, thread="threadIdx.y"):
                                        for li_0_lj_0_fused_1 in T.thread_binding(32, thread="threadIdx.x"):
                                            for li_1 in range(2):
                                                        ...
                                                        for lj_1_0 in T.unroll(2):
                                                            for lj_1_1 in T.vectorized(4):
                                                                with T.sblock("O_gemm_update"):
                                                                    i = T.axis.spatial(32, (li_0_lj_0_fused_0 * 32 + li_0_lj_0_fused_1) // 16 * 4 + li_1)
                                                                    j = T.axis.spatial(128, (li_0_lj_0_fused_0 * 32 + li_0_lj_0_fused_1) % 16 * 8 + lj_1_0 * 4 + lj_1_1)
                                                                    k_ax = T.axis.reduce(32, lk_0 * 16 + lk_1)
                                                                    T.reads(O_local[i, j], m_prev_smem[i], m_smem[i], S_smem[i, k_ax], V_smem[k_ax, j])
                                                                    T.writes(O_local[i, j])
                                                                    O_local[i, j] = O_local[i, j] + S_smem[i, k_ax] * T.Cast("float32", V_smem[k_ax, j])
                                for li_0_lj_0_fused_0 in T.thread_binding(4, thread="threadIdx.y"):
                                    for li_0_lj_0_fused_1 in T.thread_binding(32, thread="threadIdx.x"):
                                        for li_1 in range(4):
                                            for lj_1_0 in T.unroll(2):
