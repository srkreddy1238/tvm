...
# under the License.
"""Test for _attention_sequence_prefill - MQA h_kv=1, h_q=4, d=128, float16, non-causal."""
...
class TestAttentionSequencePrefillGPU_MQA_F16_NonCausal:
    @pytest.fixture(autouse=True)
    def _target(self):
        self.target = tvm.target.Target("cuda")

    def test_mqa_f16_non_causal(self):
        fn = _attention_sequence_prefill(1, 4, 128, "float16", self.target, causal=0)
        ...
        def batch_sequence_prefill_kv(var_q: T.handle, var_k: T.handle, var_v: T.handle, var_output: T.handle, var_lse: T.handle):
            ...
            q = T.match_buffer(var_q, (batch_size, qo_len, 4, 128), "float16")
            kv_len = T.int32(is_size_var=True)
            k = T.match_buffer(var_k, (batch_size, kv_len, 1, 128), "float16")
            v = T.match_buffer(var_v, (batch_size, kv_len, 1, 128), "float16")
            output = T.match_buffer(var_output, (batch_size, qo_len, 4, 128), "float16")
            lse = T.match_buffer(var_lse, (batch_size, qo_len, 4), "float16")
            # with T.sblock("root"):
            batch_tiles: T.int32 = (qo_len * 4 + 32 - 1) // 32
            for lbx in T.thread_binding(batch_size * batch_tiles, thread="blockIdx.x"):
                for lby in T.thread_binding(1, thread="blockIdx.y"):
                                            ...
                                            for lj_1_0 in T.unroll(2):
                                                for lj_1_1 in T.vectorized(4):
                                                    with T.sblock("O_store"):
                                                        ...
                                                        T.writes(output[b_idx, (LH_start + i) // 4, by * 4 + (LH_start + i) % 4, j])
                                                        cur_L: T.int32 = (LH_start + i) // 4
                                                        cur_H_qo: T.int32 = by * 4 + (LH_start + i) % 4
                                                        if cur_L < qo_len:
                                    ...
                                    for li_1 in T.thread_binding(4, thread="threadIdx.y"):
                                        for li_2 in T.thread_binding(32, thread="threadIdx.x"):
                                            with T.sblock("lse_store"):
                                                ...
                                                T.writes(lse[b_idx, (LH_start + i) // 4, by * 4 + (LH_start + i) % 4])
                                                cur_L: T.int32 = (LH_start + i) // 4
                                                cur_H_qo: T.int32 = by * 4 + (LH_start + i) % 4
                                                if cur_L < qo_len:
