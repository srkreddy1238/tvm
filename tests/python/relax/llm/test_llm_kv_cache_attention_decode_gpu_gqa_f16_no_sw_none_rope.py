...
# under the License.
"""Test for _attention_decode - GQA h_kv=4, h_q=32, d=128, float16, no sliding window, no rope."""
...
class TestAttentionDecodeGPU_GQA_F16_NoSW_NoneRope:
    @pytest.fixture(autouse=True)
    def _target(self):
        self.target = tvm.target.Target("cuda")

    def test_gqa_f16_no_sw_none_rope(self):
        fn = _attention_decode(4, 32, 128, "float16", False, ROPE_SCALING_NONE, self.target)
        ...
        def batch_decode_paged_kv(Q_handle: T.handle, pages_handle: T.handle, page_table_indptr_handle: T.handle, page_table_values_handle: T.handle, var_length_info: T.handle, k_rope_pos_offset_handle: T.handle, q_rope_position_handle: T.handle, output_handle: T.handle, lse_handle: T.handle, rotary_mode: T.int32, rope_scale: T.float32, rope_theta: T.float32, sm_scale: T.float32):
            ...
            pages = T.match_buffer(pages_handle, (max_num_pages, 2, 4, 16, 128), "float16", elem_offset=T.SizeVar("pages_elem_offset", "int64"))
