...
# under the License.
"""Test for _attention_decode - GQA h_kv=4, h_q=32, d=128, float16, no sliding window, llama3 rope."""
...
class TestAttentionDecodeGPU_GQA_F16_NoSW_Llama3Rope:
    @pytest.fixture(autouse=True)
    def _target(self):
        self.target = tvm.target.Target("cuda")

    def test_gqa_f16_no_sw_llama3_rope(self):
        fn = _attention_decode(4, 32, 128, "float16", False, ROPE_SCALING_LLAMA3, self.target)

        # fmt: off
        @T.prim_func
        def expected(Q_handle: T.handle, pages_handle: T.handle, page_table_indptr_handle: T.handle, page_table_values_handle: T.handle, var_length_info: T.handle, k_rope_pos_offset_handle: T.handle, q_rope_position_handle: T.handle, output_handle: T.handle, lse_handle: T.handle, rotary_mode: T.int32, rope_scale: T.float32, rope_theta: T.float32, sm_scale: T.float32):
                    ...
                    for ty in T.thread_binding(8, thread="threadIdx.y"):
                        for tx in T.thread_binding(32, thread="threadIdx.x"):
                            for tz in T.thread_binding(2, thread="threadIdx.z"):
                                with T.sblock("attn"):
                                    T.reads(page_table_indptr[bx:bx + 2], length_info[bx], q_rope_position[bx], Q[bx, fused_by_bz // 4 * 8 + fused_by_bz % 4 * 8 + ty, tx * 4 - 64:tx * 4 - 64 + 132])
                                    ...
                                    for vec in T.vectorized(4):
                                        ...
                                        Q_local[vec] = T.if_then_else(rotary_mode == 1, T.Let(T.Let(T.Cast("float16", T.cos(smoothed_freq) * T.Cast("float32", Q[bx, by * 8 + bz * 8 + ty, tx * 4 + vec]) + T.sin(smoothed_freq) * T.Cast("float32", T.if_then_else(tx * 4 + vec < 64, Q[bx, by * 8 + bz * 8 + ty, tx * 4 + vec + 64] * T.float16(-1.0), Q[bx, by * 8 + bz * 8 + ty, tx * 4 + vec - 64]))), where={smoothed_freq: T.Cast("float32", q_rope_position[batch_idx]) * rope_scale * ((T.float32(1.0) - T.max(T.float32(0.0), T.min(T.float32(1.0), T.float32(434.59909793626889) * orig_freq - T.float32(0.33333333333333331)))) * orig_freq * T.float32(0.125) + T.max(T.float32(0.0), T.min(T.float32(1.0), T.float32(434.59909793626889) * orig_freq - T.float32(0.33333333333333331))) * orig_freq)}), where={orig_freq: T.float32(1.0) / T.pow(rope_theta, T.Cast("float32", (tx * 4 + vec) * 2 % 128) / T.float32(128.0))}), Q[bx, by * 8 + bz * 8 + ty, tx * 4 + vec])
                                            ...
                                            with T.sblock("KV_load"):
                                                ...
                                                if row_g < kv_chunk_len[0]:
                                                    ...
                                                    for vec in T.vectorized(4):
                                                        orig_freq = T.float32()
                                                        smoothed_freq = T.float32()
                                                        K_smem[tile_start_s + j, tx * 4 + vec] = T.if_then_else(rotary_mode == 1, T.Let(T.Let(T.Cast("float16", T.cos(smoothed_freq) * T.Cast("float32", pages[page_no, 0, by, page_offset, tx * 4 + vec]) + T.sin(smoothed_freq) * T.Cast("float32", T.if_then_else(tx * 4 + vec < 64, pages[page_no, 0, by, page_offset, tx * 4 + vec + 64] * T.float16(-1.0), pages[page_no, 0, by, page_offset, tx * 4 + vec - 64]))), where={smoothed_freq: T.Cast("float32", k_rope_pos_offset[batch_idx] + row_g) * rope_scale * ((T.float32(1.0) - T.max(T.float32(0.0), T.min(T.float32(1.0), T.float32(434.59909793626889) * orig_freq - T.float32(0.33333333333333331)))) * orig_freq * T.float32(0.125) + T.max(T.float32(0.0), T.min(T.float32(1.0), T.float32(434.59909793626889) * orig_freq - T.float32(0.33333333333333331))) * orig_freq)}), where={orig_freq: T.float32(1.0) / T.pow(rope_theta, T.Cast("float32", (tx * 4 + vec) * 2 % 128) / T.float32(128.0))}), pages[page_no, 0, by, page_offset, tx * 4 + vec])
        ...
        # fmt: on
        tvm.ir.assert_structural_equal(fn, expected)
