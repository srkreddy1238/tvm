...
# under the License.
"""Test for _attention_decode - MHA h_kv=8, h_q=8, d=64, float32, no sliding window, no rope."""
...
class TestAttentionDecodeGPU_MHA_F32_NoSW_NoneRope:
    @pytest.fixture(autouse=True)
    def _target(self):
        self.target = tvm.target.Target("cuda")

    def test_mha_f32_no_sw_none_rope(self):
        fn = _attention_decode(8, 8, 64, "float32", False, ROPE_SCALING_NONE, self.target)

        # fmt: off
        # from tvm.script import tir as T

        @T.prim_func
        def batch_decode_paged_kv(Q_handle: T.handle, pages_handle: T.handle, page_table_indptr_handle: T.handle, page_table_values_handle: T.handle, var_length_info: T.handle, k_rope_pos_offset_handle: T.handle, q_rope_position_handle: T.handle, output_handle: T.handle, lse_handle: T.handle, rotary_mode: T.int32, rope_scale: T.float32, rope_theta: T.float32, sm_scale: T.float32):
            T.func_attr({"tir.is_scheduled": True, "global_symbol": "batch_decode_paged_kv"})
            ...
            Q = T.match_buffer(Q_handle, (B, 8, 64))
            pages = T.match_buffer(pages_handle, (max_num_pages, 2, 8, 16, 64), elem_offset=pages_elem_offset)
            ...
            output = T.match_buffer(output_handle, (B, 8, 64))
            lse = T.match_buffer(lse_handle, (B, 8))
            # with T.sblock("root"):
                    ...
                    for ty in T.thread_binding(1, thread="threadIdx.y"):
                        for tx in T.thread_binding(16, thread="threadIdx.x"):
                            for tz in T.thread_binding(32, thread="threadIdx.z"):
                                with T.sblock("attn"):
                                    T.reads(page_table_indptr[bx:bx + 2], length_info[bx], q_rope_position[bx], Q[bx, fused_by_bz // 8 + ty + fused_by_bz % 8, tx * 4 - 32:tx * 4 - 32 + 68])
                                    T.writes(output[bx, fused_by_bz % 8 + fused_by_bz // 8 + ty, tx * 4:tx * 4 + 4], lse[bx, fused_by_bz % 8 + fused_by_bz // 8 + ty])
                                    Q_local = T.alloc_buffer((4,), scope="local")
                                    kv_chunk_len = T.alloc_buffer((1,), "int32", scope="local")
                                    K_smem = T.alloc_buffer((64, 64), scope="shared")
                                    V_smem = T.alloc_buffer((64, 64), scope="shared")
                                    ...
                                    V_local = T.alloc_buffer((4,), scope="local")
                                    ...
                                    for vec in T.vectorized(4):
                                        freq = T.float32()
                                        Q_local[vec] = T.if_then_else(rotary_mode == 1, T.Let(T.cos(freq) * Q[bx, by + bz + ty, tx * 4 + vec] + T.sin(freq) * T.if_then_else(tx * 4 + vec < 32, Q[bx, by + bz + ty, tx * 4 + vec + 32] * T.float32(-1.0), Q[bx, by + bz + ty, tx * 4 + vec - 32]), where={freq: T.Cast("float32", q_rope_position[batch_idx]) * rope_scale / T.pow(rope_theta, T.Cast("float32", (tx * 4 + vec) * 2 % 64) / T.float32(64.0))}), Q[bx, by + bz + ty, tx * 4 + vec])
                                    for iterator in range((kv_chunk_len[0] + 63) // 64):
                                        tile_start_s: T.int32(is_size_var=True) = (tz + ty) * 2
                                        tile_start_g: T.int32(is_size_var=True) = (iterator * 32 + tz + ty) * 2
                                        for j in range(2):
                                            with T.sblock("KV_load"):
                                                ...
                                                if row_g < kv_chunk_len[0]:
                                                    seq_offset: T.int32(is_size_var=True) = row_g
                                                    ...
                                                    for vec in T.vectorized(4):
                                                        freq = T.float32()
                                                        K_smem[tile_start_s + j, tx * 4 + vec] = T.if_then_else(rotary_mode == 1, T.Let(T.cos(freq) * pages[page_no, 0, by, page_offset, tx * 4 + vec] + T.sin(freq) * T.if_then_else(tx * 4 + vec < 32, pages[page_no, 0, by, page_offset, tx * 4 + vec + 32] * T.float32(-1.0), pages[page_no, 0, by, page_offset, tx * 4 + vec - 32]), where={freq: T.Cast("float32", k_rope_pos_offset[batch_idx] + row_g) * rope_scale / T.pow(rope_theta, T.Cast("float32", (tx * 4 + vec) * 2 % 64) / T.float32(64.0))}), pages[page_no, 0, by, page_offset, tx * 4 + vec])
                                                        V_smem[tile_start_s + j, tx * 4 + vec] = pages[page_no, 1, by, page_offset, tx * 4 + vec]
                                                else:
                                                    for vec in T.vectorized(4):
                                                        K_smem[tile_start_s + j, tx * 4 + vec] = T.float32(0.0)
                                                        V_smem[tile_start_s + j, tx * 4 + vec] = T.float32(0.0)
                                        T.tvm_storage_sync("shared")
                                        m_prev[0] = st_m[0]
                                        for j in range(2):
                                            for vec in T.vectorized(4):
                                                QK_local[vec] = Q_local[vec] * K_smem[tz * 2 + j, tx * 4 + vec] * sm_scale * T.float32(1.4426950408889634)
                                        ...
                                        for j in range(2):
                                            ...
                                            for vec in T.vectorized(4):
                                                O_local[vec] = O_local[vec] + V_local[vec] * S_local[j]
                                    for vec in T.vectorized(4):
                                    ...
                                    for vec in T.vectorized(4):
                                        output[batch_idx, by + bz + ty, tx * 4 + vec] = O_local[vec]
                                    lse[batch_idx, by + bz + ty] = st_m[0] + T.log2(st_d[0])
        # fmt: on
        tvm.ir.assert_structural_equal(fn, batch_decode_paged_kv)
