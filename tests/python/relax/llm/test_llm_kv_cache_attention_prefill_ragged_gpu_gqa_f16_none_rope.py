...
# under the License.
"""Test for _attention_prefill_ragged - GQA h_kv=4, h_q=32, d_qk=128, d_v=128, float16, no rope."""
...
class TestAttentionPrefillRaggedGPU_GQA_F16_NoneRope:
    @pytest.fixture(autouse=True)
    def _target(self):
        self.target = tvm.target.Target("cuda")

    def test_gqa_f16_none_rope(self):
        fn = _attention_prefill_ragged(4, 32, 128, 128, "float16", ROPE_SCALING_NONE, self.target)
        ...
        tvm.ir.assert_structural_equal(fn, batch_prefill_ragged_kv, map_free_vars=True)
