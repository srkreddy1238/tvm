...
# under the License.
"""Test for _attention_prefill_mla - num_heads=16, v_head_dim=128, qk_nope_head_dim=128, float16, non-causal."""

import pytest

import tvm
import tvm.testing
from tvm.relax.frontend.nn.llm.kv_cache import _attention_prefill_mla
from tvm.script import tir as T


class TestAttentionPrefillMlaGPU_F16_NopeEqV:
    @pytest.fixture(autouse=True)
    def _target(self):
        self.target = tvm.target.Target("cuda")

    def test_f16_nope_eq_v(self):
        fn = _attention_prefill_mla(16, 128, 128, "float16", False, self.target)

        # fmt: off
        # from tvm.script import tir as T

        @T.prim_func
        def batch_prefill_paged_kv_mla(var_q: T.handle, var_q_indptr: T.handle, var_pages: T.handle, var_page_indptr: T.handle, var_page_values: T.handle, var_length_info: T.handle, var_output: T.handle, var_lse: T.handle, causal: T.int32, sm_scale: T.float32):
            T.func_attr({"tir.is_scheduled": True, "global_symbol": "batch_prefill_paged_kv_mla"})
            ...
            q = T.match_buffer(var_q, (total_len, 16, 256), "float16")
            q_indptr = T.match_buffer(var_q_indptr, (batch_size + 1,), "int32", elem_offset=q_indptr_elem_offset)
            pages = T.match_buffer(var_pages, (max_num_pages, 16, 256), "float16", elem_offset=pages_elem_offset)
                ...
                for lty in T.thread_binding(4, thread="threadIdx.y"):
                    for ltx in T.thread_binding(32, thread="threadIdx.x"):
                        with T.sblock("attn"):
                            ...
                            Q_smem = T.alloc_buffer((16, 256), "float16", scope="shared")
                            KV_smem = T.alloc_buffer((16, 256), "float16", scope="shared")
                            S_smem = T.alloc_buffer((16, 16), scope="shared")
                            S_local = T.alloc_buffer((16, 16), scope="local")
                            O_local = T.alloc_buffer((16, 128), scope="local")
                            ...
                            tile_id[0] = bx
                            batch_idx[0] = 0
                            batch_rows[0] = (q_indptr[1] - q_indptr[0]) * 16
                            ...
                            while T.tvm_thread_invariant(batch_idx[0] < batch_size):
                                while tile_id[0] >= batch_tiles[0] and batch_idx[0] < batch_size:
                                    ...
                                    if batch_idx[0] < batch_size:
                                        ...
                                        batch_rows[0] = (q_indptr[b_idx + 1] - q_indptr[b_idx]) * 16
                                        batch_tiles[0] = (batch_rows[0] + 16 - 1) // 16
                                if T.tvm_thread_invariant(batch_idx[0] < batch_size):
                                                ...
                                                for lj_1_0 in T.unroll(1):
                                                    for lj_1_1 in T.vectorized(4):
                                                        with T.sblock("O_init"):
                                                            i = T.axis.spatial(16, (li_0_lj_0_fused_0 * 32 + li_0_lj_0_fused_1) // 32 * 4 + li_1)
                                                            j = T.axis.spatial(128, (li_0_lj_0_fused_0 * 32 + li_0_lj_0_fused_1) % 32 * 4 + lj_1_0 * 4 + lj_1_1)
                                                            T.reads()
                                            ...
                                            for li_0, lj_0_0 in T.grid(2, 4):
                                                for lj_1 in T.vectorized(4):
                                                    with T.sblock("Q_load"):
                                                        ...
                                                        j = T.axis.spatial(256, lj_0_0 * 64 + (li_1_lj_0_1_fused_0 * 32 + li_1_lj_0_1_fused_1) % 16 * 4 + lj_1)
                                                        T.reads()
                                                        T.writes()
                                                        cur_L: T.int32 = q_indptr_val + (LH_start + i) // 16
                                                        cur_H_qo: T.int32 = (LH_start + i) % 16
                                    ...
                                    for iterator_1 in range((kv_chunk_len[0] + 15) // 16):
                                                ...
                                                for lz_0, ly_0_0 in T.grid(2, 4):
                                                    for ly_1 in T.vectorized(4):
                                                        with T.sblock("KV_load"):
                                                            ...
                                                            j = T.axis.spatial(256, ly_0_0 * 64 + (lz_1_ly_0_1_fused_0 * 32 + lz_1_ly_0_1_fused_1) % 16 * 4 + ly_1)
                                                            T.reads()
                                                            T.writes()
                                                            ...
                                                            if cur_L < kv_chunk_len[0]:
                                                                ...
                                                                KV_smem[i, j] = T.float16(0.0)
                                        T.tvm_storage_sync("shared")
                                        with T.sblock(""):
                                            T.reads(Q_smem[0:16, 0:256], KV_smem[0:16, 0:256])
                                                                ...
                                                                for lj_1_1 in T.vectorized(2):
                                                                    for lk_1 in range(16):
                                                                        with T.sblock("S_gemm_update"):
                                                                            ...
                                                                            k = T.axis.reduce(256, lk_0 * 16 + lk_1)
                                                                            T.reads(S_local[i, j], Q_smem[i, k], KV_smem[j, k])
                                                                            T.writes(S_local[i, j])
                                                                            S_local[i, j] = S_local[i, j] + T.Cast("float32", Q_smem[i, k]) * T.Cast("float32", KV_smem[j, k]) * sm_scale * T.float32(1.4426950408889634)
                                        T.tvm_storage_sync("shared")
                                        for li_0_lj_0_fused_0 in T.thread_binding(4, thread="threadIdx.y"):
                                            for li_0_lj_0_fused_1 in T.thread_binding(32, thread="threadIdx.x"):
                                                for li_1 in range(1):
                                        ...
                                        T.tvm_storage_sync("shared")
                                        with T.sblock(""):
                                            T.reads(m_prev_smem[0:16], m_smem[0:16], S_smem[0:16, 0:16], KV_smem[0:16, 0:128])
                                            T.writes(O_local[0:16, 0:128])
                                            for li_0_lj_0_fused_0_init in T.thread_binding(4, thread="threadIdx.y"):
                                                for li_0_lj_0_fused_1_init in T.thread_binding(32, thread="threadIdx.x"):
                                                        ...
                                                        for lj_1_0_init in T.unroll(1):
                                                            for lj_1_1_init in T.vectorized(4):
                                                                with T.sblock("O_gemm_init"):
                                                                    i = T.axis.spatial(16, (li_0_lj_0_fused_0_init * 32 + li_0_lj_0_fused_1_init) // 32 * 4 + li_1_init)
                                                                    j = T.axis.spatial(128, (li_0_lj_0_fused_0_init * 32 + li_0_lj_0_fused_1_init) % 32 * 4 + lj_1_0_init * 4 + lj_1_1_init)
                                                                    T.reads()
                                                                    T.writes(O_local[i, j])
                                                            ...
                                                            for lj_1_0 in T.unroll(1):
                                                                for lj_1_1 in T.vectorized(4):
                                                                    with T.sblock("O_gemm_update"):
                                                                        i = T.axis.spatial(16, (li_0_lj_0_fused_0 * 32 + li_0_lj_0_fused_1) // 32 * 4 + li_1)
                                                                        j = T.axis.spatial(128, (li_0_lj_0_fused_0 * 32 + li_0_lj_0_fused_1) % 32 * 4 + lj_1_0 * 4 + lj_1_1)
                                                                        k = T.axis.reduce(16, lk_0 * 16 + lk_1)
                                                                        T.reads(O_local[i, j], m_prev_smem[i], m_smem[i], S_smem[i, k], KV_smem[k, j])
                                                                        T.writes(O_local[i, j])
                                                                        O_local[i, j] = O_local[i, j] + S_smem[i, k] * T.Cast("float32", KV_smem[k, j])
                                    for li_0_lj_0_fused_0 in T.thread_binding(4, thread="threadIdx.y"):
                                        for li_0_lj_0_fused_1 in T.thread_binding(32, thread="threadIdx.x"):
                                            for li_1 in range(4):
                                                for lj_1_0 in T.unroll(1):
                                                    for lj_1_1 in T.vectorized(4):
                                                        with T.sblock("O_store"):
                                                            ...
                                                            T.writes(output[q_indptr[b_idx] + (LH_start + i) // 16, (LH_start + i) % 16, j])
                                                            cur_L: T.int32 = q_indptr[b_idx] + (LH_start + i) // 16
                                                            cur_H_qo: T.int32 = (LH_start + i) % 16
                                                            if cur_L < q_indptr[b_idx + 1]:
                                        ...
                                        for li_1 in T.thread_binding(4, thread="threadIdx.y"):
                                            for li_2 in T.thread_binding(32, thread="threadIdx.x"):
                                                with T.sblock("lse_store"):
                                                    ...
                                                    T.writes(lse[q_indptr[b_idx] + (LH_start + i) // 16, (LH_start + i) % 16])
                                                    cur_L: T.int32 = q_indptr[b_idx] + (LH_start + i) // 16
                                                    cur_H_qo: T.int32 = (LH_start + i) % 16
                                                    if cur_L < q_indptr[b_idx + 1]:
        ...
        # fmt: on
        tvm.ir.assert_structural_equal(fn, batch_prefill_paged_kv_mla)
