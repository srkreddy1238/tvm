...
# under the License.
"""Test for _attention_prefill_mla - num_heads=8, v_head_dim=64, qk_nope_head_dim=32, float32, non-causal."""
...
class TestAttentionPrefillMlaGPU_F32_NonCausal:
    @pytest.fixture(autouse=True)
    def _target(self):
        self.target = tvm.target.Target("cuda")

    def test_f32_non_causal(self):
        fn = _attention_prefill_mla(8, 64, 32, "float32", False, self.target)
        ...
        def batch_prefill_paged_kv_mla(var_q: T.handle, var_q_indptr: T.handle, var_pages: T.handle, var_page_indptr: T.handle, var_page_values: T.handle, var_length_info: T.handle, var_output: T.handle, var_lse: T.handle, causal: T.int32, sm_scale: T.float32):
            ...
            q = T.match_buffer(var_q, (total_len, 8, 96))
            q_indptr = T.match_buffer(var_q_indptr, (batch_size + 1,), "int32", elem_offset=q_indptr_elem_offset)
            pages = T.match_buffer(var_pages, (max_num_pages, 16, 96), elem_offset=pages_elem_offset)
                ...
                for lty in T.thread_binding(4, thread="threadIdx.y"):
                    for ltx in T.thread_binding(32, thread="threadIdx.x"):
                        with T.sblock("attn"):
                            ...
                            Q_smem = T.alloc_buffer((16, 96), scope="shared")
                            KV_smem = T.alloc_buffer((16, 96), scope="shared")
                            ...
                            while T.tvm_thread_invariant(batch_idx[0] < batch_size):
                                ...
                                if T.tvm_thread_invariant(batch_idx[0] < batch_size):
                                            ...
                                            for li_0, lj_0_0 in T.grid(2, 3):
                                                for lj_1 in T.vectorized(2):
                                                    with T.sblock("Q_load"):
                                                        ...
                                                        j = T.axis.spatial(96, lj_0_0 * 32 + (li_1_lj_0_1_fused_0 * 32 + li_1_lj_0_1_fused_1) % 16 * 2 + lj_1)
                                                        T.reads()
                                                        T.writes()
                                                        cur_L: T.int32 = q_indptr_val + (LH_start + i) // 8
                                                        cur_H_qo: T.int32 = (LH_start + i) % 8
                                    ...
                                    for iterator_1 in range((kv_chunk_len[0] + 15) // 16):
                                                ...
                                                for lz_0, ly_0_0 in T.grid(2, 3):
                                                    for ly_1 in T.vectorized(2):
                                                        with T.sblock("KV_load"):
                                                            ...
                                                            j = T.axis.spatial(96, ly_0_0 * 32 + (lz_1_ly_0_1_fused_0 * 32 + lz_1_ly_0_1_fused_1) % 16 * 2 + ly_1)
                                                            T.reads()
                                                            T.writes()
                                                            ...
                                                            if cur_L < kv_chunk_len[0]:
                                                                ...
                                                                KV_smem[i, j] = T.float32(0.0)
                                        T.tvm_storage_sync("shared")
                                        with T.sblock(""):
                                            T.reads(Q_smem[0:16, 0:96], KV_smem[0:16, 0:96])
                                                                ...
                                                                for lj_1_1 in T.vectorized(2):
                                                                    for lk_1 in range(16):
                                                                        with T.sblock("S_gemm_update"):
                                                                            ...
                                                                            T.writes(S_local[i, j])
                                                                            S_local[i, j] = S_local[i, j] + Q_smem[i, k] * KV_smem[j, k] * sm_scale * T.float32(1.4426950408889634)
                                        T.tvm_storage_sync("shared")
                                        for li_0_lj_0_fused_0 in T.thread_binding(4, thread="threadIdx.y"):
                                            for li_0_lj_0_fused_1 in T.thread_binding(32, thread="threadIdx.x"):
                                                for li_1 in range(1):
                                        ...
                                        T.tvm_storage_sync("shared")
                                        with T.sblock(""):
                                            T.reads(m_prev_smem[0:16], m_smem[0:16], S_smem[0:16, 0:16], KV_smem[0:16, 0:64])
                                                            ...
                                                            for lj_1_0 in T.unroll(2):
                                                                for lj_1_1 in T.vectorized(2):
                                                                    with T.sblock("O_gemm_update"):
                                                                        ...
                                                                        T.writes(O_local[i, j])
                                                                        O_local[i, j] = O_local[i, j] + S_smem[i, k] * KV_smem[k, j]
                                    for li_0_lj_0_fused_0 in T.thread_binding(4, thread="threadIdx.y"):
                                        for li_0_lj_0_fused_1 in T.thread_binding(32, thread="threadIdx.x"):
                                            for li_1 in range(2):
                                                for lj_1_0 in T.unroll(2):
                                                    for lj_1_1 in T.vectorized(2):
                                                        with T.sblock("O_store"):
                                                            ...
                                                            T.writes(output[q_indptr[b_idx] + (LH_start + i) // 8, (LH_start + i) % 8, j])
                                        ...
                                        for li_1 in T.thread_binding(4, thread="threadIdx.y"):
                                            for li_2 in T.thread_binding(32, thread="threadIdx.x"):
                                                with T.sblock("lse_store"):
                                                    ...
                                                    T.writes(lse[q_indptr[b_idx] + (LH_start + i) // 8, (LH_start + i) % 8])
                                                    cur_L: T.int32 = q_indptr[b_idx] + (LH_start + i) // 8
                                                    cur_H_qo: T.int32 = (LH_start + i) % 8
                                                    if cur_L < q_indptr[b_idx + 1]:
