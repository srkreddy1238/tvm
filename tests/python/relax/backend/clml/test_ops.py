...
def test_conv2d_offload(
    ...
    bias = np.random.randint(low, high, size=(1, weight_shape[0], 1, 1)).astype(dtype)
    ...
    mean = np.random.randint(low, high, size=(weight_shape[0],)).astype(dtype)
    variance = np.random.randint(low, high, size=(weight_shape[0],)).astype(dtype)
    ...
    out, ref = run_compare(mod, clml_mod, inputs, params_np, rpc, clml_codegen)
...
def test_conv2d_transpose(
    ...
    low, high = -1, 1
    data = np.random.randint(low, high, size=dshape).astype(dtype)
    weight = np.random.randint(low, high, size=kshape).astype(dtype)
...
def test_batchnorm(dtype, trials, rpc):
    ...
    out, ref = run_compare(mod, clml_mod, inputs, params_np, rpc, exp_codegen)
    np.testing.assert_allclose(out, ref, rtol=1e-5, atol=1e-5)
...
def test_binary_ops(a_shape, b_shape, op, rpc, dtype):
    ...
    def _verify(mod, clml_mod, inputs):
        ...
        out, ref = run_compare(mod, clml_mod, inputs, {}, rpc, expected_codegen_str)
        np.testing.assert_allclose(out, ref, rtol=1e-5, atol=1e-5)
...
def test_max_pool(dtype, trials, rpc):
    ...
    data = np.random.randint(low, high, size=input_shape).astype(dtype)
    inputs = [data]
    ...
    out, ref = run_compare(mod, clml_mod, inputs, params_np, rpc, expected_codegen_str)
    np.testing.assert_allclose(out, ref, rtol=1e-5, atol=1e-5)
...
def test_avg_pool(dtype, trials, rpc):
    ...
    out, ref = run_compare(mod, clml_mod, inputs, params_np, rpc, exp_codegen_str)
    np.testing.assert_allclose(out, ref, rtol=1e-5, atol=1e-5)
...
def test_reshape(dtype, trials, rpc):
    ...
    out, ref = run_compare(mod, clml_mod, inputs, params_np, rpc, expected_codegen)
    np.testing.assert_allclose(out, ref, rtol=1e-5, atol=1e-5)
