...
// CPP Compiler tests - Statistical

#include <tvm/relax/attrs/statistical.h>

#include "compiler_base.h"

class Statistical : public ::testing::TestWithParam<
                        std::tuple<std::tuple<DLDeviceType, std::string, std::string, std::string>,
                                   std::tuple<DLDataType, std::string>, bool>>,
  ...
  void SetUp() override {
    ...
    relax_op = std::get<1>(op_spec);
    is_symbolic = std::get<2>(GetParam());
  }
...
INSTANTIATE_TEST_SUITE_P(
    CPPCompierStatistical, Statistical,
    ::testing::Combine(
        ::testing::ValuesIn(
        ...
        ::testing::ValuesIn(std::vector<std::tuple<DLDataType, std::string>>{
            ...
            std::make_tuple(DLDataType({kDLFloat, 32, 1}), "max"),
            std::make_tuple(DLDataType({kDLInt, 32, 1}), "max"),
            std::make_tuple(DLDataType({kDLFloat, 32, 1}), "sum"),
            std::make_tuple(DLDataType({kDLInt, 32, 1}), "sum"),
            std::make_tuple(DLDataType({kDLFloat, 32, 1}), "prod"),
            std::make_tuple(DLDataType({kDLInt, 32, 1}), "prod"),
            std::make_tuple(DLDataType({kDLFloat, 32, 1}), "mean"),
            std::make_tuple(DLDataType({kDLInt, 32, 1}), "mean"),
            std::make_tuple(DLDataType({kDLFloat, 32, 1}), "variance"),
            std::make_tuple(DLDataType({kDLInt, 32, 1}), "variance"),
            std::make_tuple(DLDataType({kDLFloat, 32, 1}), "std"),
        }),
