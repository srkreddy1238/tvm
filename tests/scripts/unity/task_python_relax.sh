...
# Run Relax tests
TVM_TEST_TARGETS="${TVM_RELAY_TEST_TARGETS:-llvm}" pytest tests/python/relax --ignore tests/python/relax/test_frontend_tflite.py
# Workaround to avoid crash
TVM_TEST_TARGETS="${TVM_RELAY_TEST_TARGETS:-llvm}" python tests/python/relax/test_frontend_tflite.py
...
# Test for MSC
IS_MSC=`python3 -c 'import tvm; print(tvm.support.libinfo().get("USE_MSC", "OFF"))'`
if [ ${IS_MSC} != "OFF" ] ; then
