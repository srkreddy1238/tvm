...
class ApplyFallbackRuleNode : public DlightRuleNode {
 public:
  ...
  virtual ffi::Optional<tir::Schedule> Apply(const tir::PrimFunc& func,
                                             const tvm::Target& target) final {
    auto keys = target->GetKeys();
    if (std::find(keys.begin(), keys.end(), "gpu") == keys.end()) {
      return std::nullopt;
    }
    ...
    auto sch = tir::Schedule::Traced(IRModule(func_map), -1, 0,
                                     tir::ScheduleErrorRenderLevel::kDetail, true);
    ...
    ffi::Map<tir::SBlockRV, tir::LoopRV> reduction_blocks;

    // Blockwise
    for (auto it = blk_infos.begin(); it != blk_infos.end(); ++it) {
      auto blk_info = *it;
      ffi::Array<tir::LoopRV> s_loops, r_loops, o_loops;
      ...
      // Reorder loops
      ffi::Array<tir::LoopRV> all_loops;
      ...
      // Bindings
      ffi::Array<ffi::Optional<tir::ExprRV>> factors = {ffi::Optional<tir::ExprRV>(),
                                                        tir::ExprRV(max_threads_per_block)};
...
ffi::Optional<tir::Schedule> ApplyFallbackRule(tir::PrimFunc func, tvm::Target target) {
