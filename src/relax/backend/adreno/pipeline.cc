...
class AdrenoRelaxPipeline : GPURelaxPipeline {
 public:
  ...
  IRModule Library(IRModule& mod) {
    // TODO: CLML Dispatches
    return mod;
  }

  IRModule Legalize(IRModule& mod) {
    auto keys = Target::Current(true)->GetKeys();
    bool is_texture = std::find(keys.begin(), keys.end(), "texture") != keys.end();
    ...
    if (is_texture) {
      // TODO: {"relax.nn.conv2d": legalize_adreno.conv2d_NCHWc_OIHWo}
