...
StructInfo InferStructInfoQnnBinary(const Call& call, const BlockBuilder& ctx,
  ...
  TVM_FFI_ICHECK(attrs) << "Attributes for QNN binary op are missing.";
  ...
  TVM_FFI_ICHECK(GetInputTensorStructInfo(call, 2, ctx)->dtype.is_float())
      << "LHS scale must be float.";
  TVM_FFI_ICHECK(GetInputTensorStructInfo(call, 4, ctx)->dtype.is_float())
      << "RHS scale must be float.";
  TVM_FFI_ICHECK(GetInputTensorStructInfo(call, 6, ctx)->dtype.is_float())
      << "Output scale must be float.";
  ...
  DataType out_dtype = f_compute_out_dtype(call, ctx, GetInputTensorStructInfo(call, 0, ctx),
                                           GetInputTensorStructInfo(call, 1, ctx),
                                           GetInputTensorStructInfo(call, 7, ctx));

  // Compute output shape using broadcast rules
  auto output_shape = InferBinaryBroadcastShape(
      call, ctx, (GetInputTensorStructInfo(call, 0, ctx)->GetShape()).value(),
...
#define RELAX_QNN_BINARY_ARITH_OP(OpName)                                                          \
  Expr OpName(Expr lhs, Expr rhs, Expr lhs_scale, Expr lhs_zero_point, Expr rhs_scale,             \
    ...
    return Call(op,                                                                                \
                {std::move(lhs), std::move(rhs), std::move(lhs_scale), std::move(lhs_zero_point),  \
                 std::move(rhs_scale), std::move(rhs_zero_point), std::move(output_scale),         \
                 std::move(output_zero_point)},                                                    \
                Attrs(attrs), {});                                                                 \
  }                                                                                                \
  ...
  TVM_REGISTER_OP("relax.qnn." #OpName)                                                            \
      ...
      .add_argument("lhs_zero_point", "Tensor", "Zero point for dequantizing the LHS tensor.")     \
      .add_argument("rhs_scale", "Tensor", "Scale for dequantizing the RHS tensor.")               \
      .add_argument("rhs_zero_point", "Tensor", "Zero point for dequantizing the RHS tensor.")     \
      .add_argument("output_scale", "Tensor", "Scale for quantizing the output tensor.")           \
      .add_argument("output_zero_point", "Tensor", "Zero point for quantizing the output tensor.") \
