...
DecodeGpuConfig ComputeDecodeGpuConfig(int64_t H_kv, int64_t H_qo, int64_t D,
  ...
  int64_t VEC_SIZE = std::min(std::max(static_cast<int64_t>(8LL / qkv_dtype_bytes), D / 32),
                              static_cast<int64_t>(4LL));
...
PrimExpr BuildDecodeRopeExpr(tir::Buffer buf, ffi::Array<PrimExpr> base_indices, PrimExpr d_expr,
  ...
  PrimExpr partner =
      tvm::if_then_else(d_expr < I32(half_d), tir::BufferLoad(buf, idx_plus) * neg_one,
...
Stmt BuildDecodeKVLoadBody(const DecodeGpuConfig& cfg, tir::Buffer pages_buf,
  ...
  if (sliding_window) {
    seq_offset_expr =
        tvm::if_then_else(row_g < tir::BufferLoad(length_info_buf, {I32(2), batch_idx}), row_g,
  ...
  PrimExpr k_rope =
      BuildDecodeRopeExpr(pages_buf, ffi::Array<PrimExpr>{page_no, I32(0), by_var, page_offset_var},
                          d_kv, pages_buf->shape[4].as<IntImmNode>()->value, k_pos, rope_scale,
                          rope_theta, rotary_mode, pages_dtype_str, rope_scaling);
  ...
  Stmt zero_branch =
      tir::For(vec0, I32(0), I32(cfg.VEC_SIZE), tir::ForKind::kVectorized,
               tir::SeqStmt({tir::BufferStore(K_smem, zero_kv, {tile_start_s + j_var, d_zero}),
                             tir::BufferStore(V_smem, zero_kv, {tile_start_s + j_var, d_zero})}));

  // Wrap in row_g LetStmt
  return tir::LetStmt(row_g, row_g_expr, tir::IfThenElse(row_g < kv_len, load_branch, zero_branch));
}
...
Stmt BuildDecodeAllreduceBlock(const DecodeGpuConfig& cfg, tir::Buffer O_allreduce,
  ...
  Stmt store_md = tir::SeqStmt({tir::BufferStore(md_allreduce, ld0(st_m), {tz, ty, I32(0)}),
  ...
  Stmt reset =
      tir::SeqStmt({st0(st_m, F32(-50000.0)), st0(st_d, F32(1.0)),
