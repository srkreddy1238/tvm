...
static PrimExpr MakeFloat32(double value) { return FloatImm(DataType::Float(32), value); }
...
RopeFreqResult RopeFreqDefault(PrimExpr s, PrimExpr d, int64_t d_range, PrimExpr theta,
  ...
  PrimExpr exponent =
      CastTo(tir::FloorMod(d * IntImm(d.dtype(), 2), IntImm(d.dtype(), d_range)), "float32") /
...
RopeFreqResult RopeFreqGptj(PrimExpr s, PrimExpr d, int64_t d_range, PrimExpr theta,
  ...
  PrimExpr exponent =
      CastTo(tir::FloorMod(IntImm(d.dtype(), 2) * tir::FloorDiv(d, IntImm(d.dtype(), 2)),
                           IntImm(d.dtype(), d_range)),
             "float32") /
      MakeFloat32(static_cast<double>(d_range));
  PrimExpr freq = s / tvm::pow(theta, exponent);
...
RopeFreqResult RopeFreqLlama3(PrimExpr s, PrimExpr d, int64_t d_range, PrimExpr theta,
  ...
  PrimExpr smooth = tvm::max(MakeFloat32(0.0),
                             tvm::min(MakeFloat32(1.0), MakeFloat32(llama3_alpha) * orig_freq_var -
                                                            MakeFloat32(llama3_beta)));

  // smoothed_freq = s * ((1 - smooth) * orig_freq * inv_scaling_factor + smooth * orig_freq)
  // s is already float32 by convention
  PrimExpr smoothed_freq =
      s * ((MakeFloat32(1.0) - smooth) * orig_freq_var * MakeFloat32(llama3_inv_scaling_factor) +
...
RopeFreqResult RopeFreqLlama4(PrimExpr s, PrimExpr d, int64_t d_range, PrimExpr theta,
  ...
  PrimExpr exponent =
      CastTo(IntImm(d.dtype(), 2) * tir::FloorDiv(d, IntImm(d.dtype(), 2)), "float32") /
      MakeFloat32(static_cast<double>(d_range));
  PrimExpr orig_freq = MakeFloat32(1.0) / tvm::pow(theta, exponent);
  ...
  if (high_freq_factor == low_freq_factor) {
    ...
    PrimExpr smooth = tvm::max(
        MakeFloat32(0.0), tvm::min(MakeFloat32(1.0), MakeFloat32(llama4_alpha) * orig_freq_var -
                                                         MakeFloat32(llama4_beta)));

    // s is already float32 by convention
    smoothed_freq =
        s * ((MakeFloat32(1.0) - smooth) * orig_freq_var * MakeFloat32(llama4_inv_scaling_factor) +
             smooth * orig_freq_var);
  }
...
RopeFreqResult RopeFreqLongrope(PrimExpr s, PrimExpr d, int64_t d_range, PrimExpr theta,
  ...
  double scaling_factor =
      (scale > 1.0) ? std::sqrt(1.0 + std::log(scale) / std::log(original_max_position_embeddings))
                    : 1.0;
...
static std::pair<PrimExpr, PrimExpr> YarnFindCorrectionRange(PrimExpr d, int64_t low_rot,
                                                             int64_t high_rot, int64_t d_range,
...
std::tuple<NNTensor, NNTensor, NNTensor> LlamaRope(
  ...
  tir::Buffer qkv_buf =
      tir::decl_buffer({batch_var, seq_var, I32(fused_heads), I32(head_dim)}, dtype, "qkv");
  tir::Buffer q_buf =
      tir::decl_buffer({batch_var, seq_var, I32(num_q_heads), I32(head_dim)}, dtype, "q");
  tir::Buffer k_buf =
      tir::decl_buffer({batch_var, seq_var, I32(num_kv_heads), I32(head_dim)}, dtype, "k");
  tir::Buffer v_buf =
      tir::decl_buffer({batch_var, seq_var, I32(num_kv_heads), I32(head_dim)}, dtype, "v");
  ...
  auto build_rope_expr = [&]() -> PrimExpr {
    ...
    auto freq_result =
        rope_freq_func(pos_f32, d, rotary_dim_val, MakeFloat32(theta), dtype_str, rope_scaling);
    ...
    if (rope_type == "gptj") {
      sin_part = freq_result.sin_freq *
                 tvm::if_then_else(tir::FloorMod(d, I32(2)) == I32(0),
    ...
    for (const auto& [var, expr] : freq_result.var_map) result = tir::Let(var, expr, result);
    return result;
  };

  Stmt q_store = tir::BufferStore(q_buf,
                                  tvm::if_then_else(d < I32(rotary_dim_val), build_rope_expr(),
                                                    tir::BufferLoad(qkv_buf, {b, s, h, d})),
                                  {b, s, h, d});
  Stmt k_store = tir::BufferStore(k_buf,
                                  tvm::if_then_else(d < I32(rotary_dim_val), build_rope_expr(),
                                                    tir::BufferLoad(qkv_buf, {b, s, h, d})),
                                  {b, s, h - I32(num_q_heads), d});
  Stmt v_store = tir::BufferStore(v_buf, tir::BufferLoad(qkv_buf, {b, s, h, d}),
                                  {b, s, h - I32(num_q_heads + num_kv_heads), d});

  Stmt inner_body =
      tir::IfThenElse(h < I32(num_q_heads), q_store,
  ...
  ffi::Array<tir::IterVar> iter_vars = {
      tir::IterVar(Range::FromMinExtent(IntImm(DataType::Int(64), 0), batch_var), b, tir::kDataPar,
                   ""),
      tir::IterVar(Range::FromMinExtent(IntImm(DataType::Int(64), 0), seq_var), s, tir::kDataPar,
                   ""),
  ...
  Stmt sblock_realize = tir::SBlockRealize(
      ...
      tir::const_true(),
      tir::SBlock(iter_vars, {}, {}, "llama_fused_rope", inner_body, std::nullopt, {}, {},
                  sblock_annots));
  ...
  loop_body =
      tir::For(iters_1, IntImm(DataType::Int(64), 0), seq_var, tir::ForKind::kSerial, loop_body);
  loop_body =
      tir::For(iters_0, IntImm(DataType::Int(64), 0), batch_var, tir::ForKind::kSerial, loop_body);

  // Root sblock to match TVMScript-generated IR
  Stmt root_block =
      tir::SBlockRealize({}, tir::const_true(), tir::SBlock({}, {}, {}, "root", loop_body));
  ...
  ffi::Any result_any = NNTensorIrOp(prim_func, "llama_rope", {qkv->expr, PrimValue(total_seq_len)},
...
static tir::PrimFunc BuildLlamaRopeWithPositionMapFunc(
  ...
  tir::Buffer qkv_buf =
      tir::decl_buffer({seq_len, I32(fused_heads), I32(head_dim)}, elem_dtype, "qkv");
  tir::Buffer q_buf = tir::decl_buffer({seq_len, I32(num_q_heads), I32(head_dim)}, elem_dtype, "q");
  tir::Buffer k_buf =
      tir::decl_buffer({seq_len, I32(num_kv_heads), I32(head_dim)}, elem_dtype, "k");
  tir::Buffer v_buf =
      tir::decl_buffer({seq_len, I32(num_kv_heads), I32(head_dim)}, elem_dtype, "v");
  ...
  tir::Buffer pos_buf =
      tir::Buffer(pos_data, DataType::Int(32), ffi::Array<PrimExpr>{seq_len},
                  ffi::Array<PrimExpr>{}, position_map_elem_offset, "position_map",
                  /*data_alignment=*/0,
                  /*offset_factor=*/1, tir::kDefault);
  ...
  auto make_rope_val = [&]() -> PrimExpr {
    ...
    auto freq_result =
        rope_freq_func(pos_f32, d, rotary_dim, MakeFloat32(theta), "float32", rope_scaling);
    ...
    if (rope_type == "gptj") {
      sin_part = freq_result.sin_freq *
                 CastTo(tvm::if_then_else(tir::FloorMod(d, I32(2)) == I32(0),
      ...
      sin_part =
          ...
          CastTo(tvm::if_then_else(d < I32(rotary_dim / 2),
    ...
    for (const auto& [var, expr] : freq_result.var_map) rope_val = tir::Let(var, expr, rope_val);
    return rope_val;
  };
  ...
  Stmt q_store = tir::BufferStore(
      q_buf, tvm::if_then_else(cond, make_rope_val(), tir::BufferLoad(qkv_buf, {s, h, d})),
      {s, h, d});
  // K store: h - num_q_heads
  Stmt k_store = tir::BufferStore(
      k_buf, tvm::if_then_else(cond, make_rope_val(), tir::BufferLoad(qkv_buf, {s, h, d})),
      {s, h - I32(num_q_heads), d});
  // V store: no RoPE
  Stmt v_store = tir::BufferStore(v_buf, tir::BufferLoad(qkv_buf, {s, h, d}),
  ...
  Stmt sblock_realize = tir::SBlockRealize(
      {PrimExpr(iters_0), PrimExpr(iters_1), PrimExpr(iters_2)}, tir::const_true(),
      tir::SBlock(iter_vars, {}, {}, "llama_fused_rope", inner_body, std::nullopt, {}, {},
                  sblock_annots));
...
static tir::PrimFunc BuildLlamaRopeWithPositionMapLongropeFunc(
  ...
  int64_t max_position_embeddings = rope_scaling.at("max_position_embeddings").cast<int64_t>();
  ...
  tir::Var pos_data("position_map", PointerType(PrimType(DataType::Int(32))));
  tir::Buffer pos_buf = tir::Buffer(pos_data, DataType::Int(32), {seq_len}, {},
  ...
  auto make_loop_body = [&](tir::Buffer factors_buf, const std::string& factors_name) -> Stmt {
    ...
    auto make_rope_val = [&]() -> PrimExpr {
      ...
      PrimExpr exponent = CastTo(tir::FloorMod(ld * I32(2), I32(rotary_dim)), "float32") /
                          MakeFloat32(static_cast<double>(rotary_dim));
      PrimExpr divisor = tir::BufferLoad(factors_buf, {tir::FloorMod(ld, I32(half_rotary))}) *
      ...
      PrimExpr sin_part =
          sin_v *
          CastTo(tvm::if_then_else(ld < I32(half_rotary),
    ...
    Stmt q_store = tir::BufferStore(
        q_buf, tvm::if_then_else(cond, make_rope_val(), tir::BufferLoad(qkv_buf, {ls, lh, ld})),
        {ls, lh, ld});
    Stmt k_store = tir::BufferStore(
        k_buf, tvm::if_then_else(cond, make_rope_val(), tir::BufferLoad(qkv_buf, {ls, lh, ld})),
        {ls, lh - I32(num_q_heads), ld});
    Stmt v_store = tir::BufferStore(v_buf, tir::BufferLoad(qkv_buf, {ls, lh, ld}),
                                    {ls, lh - I32(num_q_heads + num_kv_heads), ld});

    Stmt inner_body =
        tir::IfThenElse(lh < I32(num_q_heads), q_store,
    ...
    Stmt sblock =
        tir::SBlockRealize({PrimExpr(li0), PrimExpr(li1), PrimExpr(li2)}, tir::const_true(),
  ...
  tir::Buffer long_factors_buf = tir::Buffer(ext_buf->data, DataType::Float(32), {I32(half_rotary)},
                                             {}, I32(0), "long_factors", 0, 0, tir::kDefault);
  // short_factors: sub-buffer aliasing ext_factors.data, offset=half_rotary
  tir::Buffer short_factors_buf =
      tir::Buffer(ext_buf->data, DataType::Float(32), {I32(half_rotary)}, {}, I32(half_rotary),
                  "short_factors", 0, 0, tir::kDefault);
  ...
  Stmt root_body =
      tir::IfThenElse(seq_len > I64(original_max_position_embeddings), long_loop, short_loop);

  Stmt root_block =
      tir::SBlockRealize({}, tir::const_true(), tir::SBlock({}, {}, {}, "root", root_body));
  ...
  prim_func = WithAttr(prim_func, "op_pattern", ffi::Any(static_cast<int64_t>(8)));
  prim_func =
      WithAttr(prim_func, "global_symbol", ffi::Any(ffi::String("fused_rope_longrope_scaling")));
  prim_func = tir::ScriptComplete(prim_func, {});
...
tir::PrimFunc LlamaRopeWithPositionMap(double theta, double scale, int64_t head_dim,
  ...
  return BuildLlamaRopeWithPositionMapFunc(theta, scale, head_dim, num_q_heads, num_kv_heads, dtype,
                                           rope_scaling, rotary_dim_val);
}
...
TVM_FFI_STATIC_INIT_BLOCK() {
  ...
  refl::GlobalDef()
      .def("relax.frontend.nn.llm.position_embedding.switch_rope_freq_func",
           [](ffi::Map<ffi::String, ffi::Any> rope_scaling, PrimExpr s, PrimExpr d, int64_t d_range,
              PrimExpr theta, ffi::String dtype) -> ffi::Array<ffi::Any> {
             ...
             return {ffi::Any(res.cos_freq), ffi::Any(res.sin_freq), ffi::Any(keys),
                     ffi::Any(vals)};
           })
      .def("relax.frontend.nn.llm.position_embedding.llama_rope",
           [](NNTensor qkv, tir::Var total_seq_len, double theta, double scale, int64_t num_q_heads,
              int64_t num_kv_heads, ffi::Map<ffi::String, ffi::Any> rope_scaling,
              ffi::Optional<int64_t> rotary_dim) -> ffi::Array<ffi::Any> {
             auto [q, k, v] = LlamaRope(qkv, total_seq_len, theta, scale, num_q_heads, num_kv_heads,
      ...
      .def("relax.frontend.nn.llm.position_embedding.llama_rope_with_position_map",
           [](double theta, double scale, int64_t head_dim, int64_t num_q_heads,
              int64_t num_kv_heads, ffi::String dtype, ffi::Map<ffi::String, ffi::Any> rope_scaling,
