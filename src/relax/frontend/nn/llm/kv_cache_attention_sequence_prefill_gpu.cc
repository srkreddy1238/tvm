...
tir::PrimFunc AttentionSequencePrefill(int64_t h_kv, int64_t h_q, int64_t d_qk, int64_t d_v,
  ...
  tir::Buffer lse_buf = tir::decl_buffer({batch_size, qo_len, I32(h_q)}, DataType::Float(32), "lse");
  ...
  // ---- derived vars ----
  // batch_tiles = ceil(qo_len / tile_x)
  PrimExpr batch_tiles = floordiv(qo_len + I32(tile_x - 1), I32(tile_x));
  ...
  Stmt q_load = tir::For(li_ql, I32(0), I32(tile_x), tir::ForKind::kSerial,
      tir::For(lj_ql, I32(0), I32(d_qk), tir::ForKind::kSerial,
          tir::SBlockRealize({}, tir::const_true(),
              tir::SBlock({}, {}, {}, "Q_load",
                  tir::LetStmt(cur_L_ql, LH_start + li_ql,
                      tir::LetStmt(cur_H_qo_ql, by,
                          tir::IfThenElse(cur_L_ql < qo_len,
                              tir::BufferStore(Q_smem,
                                  tir::BufferLoad(q_buf, ffi::Array<PrimExpr>{b_idx, cur_L_ql, cur_H_qo_ql, lj_ql}), ffi::Array<PrimExpr>{li_ql, lj_ql}),
          ...
          tir::SBlockRealize({}, tir::const_true(),
              tir::SBlock({}, {}, {}, "K_load",
                  tir::LetStmt(cur_L_kl, L_kv_start + lz_kl,
                      tir::IfThenElse(cur_L_kl < kv_len,
                          tir::BufferStore(K_smem,
                              tir::BufferLoad(k_buf, ffi::Array<PrimExpr>{b_idx, L_kv_base + cur_L_kl, by, ly_kl}), ffi::Array<PrimExpr>{lz_kl, ly_kl}),
                          tir::BufferStore(K_smem, zero_val, ffi::Array<PrimExpr>{lz_kl, ly_kl})))))));
          ...
          tir::SBlockRealize({}, tir::const_true(),
              tir::SBlock({}, {}, {}, "V_load",
                  tir::LetStmt(cur_L_vl, L_kv_start + lz_vl,
                      tir::IfThenElse(cur_L_vl < kv_len,
                          tir::BufferStore(V_smem,
                              tir::BufferLoad(v_buf, ffi::Array<PrimExpr>{b_idx, L_kv_base + cur_L_vl, by, ly_vl}), ffi::Array<PrimExpr>{lz_vl, ly_vl}),
      ...
      tir::SBlockRealize(
          ...
          tir::SBlock(iter_vars_sg, s_gemm_reads, s_gemm_writes, "S_gemm",
              tir::BufferStore(S_local,
                  ...
                  (is_f16 ? CastTo(tir::BufferLoad(Q_smem, ffi::Array<PrimExpr>{iv_sg_i->var, iv_sg_k->var}), "float32") : tir::BufferLoad(Q_smem, ffi::Array<PrimExpr>{iv_sg_i->var, iv_sg_k->var})) * (is_f16 ? CastTo(tir::BufferLoad(K_smem, ffi::Array<PrimExpr>{iv_sg_j->var, iv_sg_k->var}), "float32") : tir::BufferLoad(K_smem, ffi::Array<PrimExpr>{iv_sg_j->var, iv_sg_k->var})) * F32(log2e),
  ...
  // update1 (non-causal: no row_ computation)
  tir::Var i_u1("i", DataType::Int(32)), row_u1("row", DataType::Int(32)), j_u1("j", DataType::Int(32));
              ...
              tir::SBlockRealize({}, tir::const_true(),
                  tir::SBlock({}, {}, {}, "update1",
                      tir::SeqStmt({
                          ...
                          tir::Evaluate(I32(0)),  // no-op (non-causal: no row_ needed)
                          tir::For(j_u1, I32(0), I32(tile_z), tir::ForKind::kSerial,
                              tir::IfThenElse(L_kv_start + j_u1 < kv_len,
  ...
  tir::Var i_u2("i", DataType::Int(32)), row_u2("row", DataType::Int(32)), j_u2("j", DataType::Int(32));
  Stmt update2_inner_ite =
      tir::IfThenElse(L_kv_start + j_u2 < kv_len,
                                  tir::BufferStore(S_smem,
  ...
  Stmt kv_loop = tir::For(iterator, I32(0),
      floordiv(kv_len + I32(tile_z - 1), I32(tile_z)),
  ...
  Stmt o_store = tir::For(li_os, I32(0), I32(tile_x), tir::ForKind::kSerial,
      tir::For(lj_os, I32(0), I32(d_v), tir::ForKind::kSerial,
          tir::SBlockRealize({}, tir::const_true(),
              tir::SBlock({}, {}, {}, "O_store",
                  tir::LetStmt(cur_L_os, LH_start + li_os,
                      tir::LetStmt(cur_H_qo_os, by,
                          tir::IfThenElse(cur_L_os < qo_len,
                              tir::BufferStore(output_buf,
                                  ...
                                  : tvm::div(tir::BufferLoad(O_local, ffi::Array<PrimExpr>{li_os, lj_os}),
                                    tir::BufferLoad(d_smem, {li_os})), ffi::Array<PrimExpr>{b_idx, cur_L_os, cur_H_qo_os, lj_os}))))))));
  ...
  Stmt lse_store = tir::For(li_lse, I32(0), I32(tile_x), tir::ForKind::kSerial,
      tir::SBlockRealize({}, tir::const_true(),
          tir::SBlock({}, {}, {}, "lse_store",
              tir::LetStmt(cur_L_lse, LH_start + li_lse,
                  tir::LetStmt(cur_H_qo_lse, by,
                      tir::IfThenElse(cur_L_lse < qo_len,
                          tir::BufferStore(lse_buf,
                              tir::BufferLoad(m_smem, {li_lse}) +
                              tvm::log2(tir::BufferLoad(d_smem, {li_lse})), ffi::Array<PrimExpr>{b_idx, cur_L_lse, cur_H_qo_lse})))))));
  ...
  Stmt sblock_body = tir::LetStmt(b_idx, floordiv(vbx, batch_tiles),
  ...
  // blockIdx.x extent = batch_size * batch_tiles
  PrimExpr bx_extent = batch_size * batch_tiles;
  Stmt body = tir::SBlockRealize({}, tir::const_true(),
      tir::SBlock({}, {}, {}, "root",
          tir::For(lbx, I32(0), bx_extent, tir::ForKind::kThreadBinding,
...
TVM_FFI_STATIC_INIT_BLOCK() {
  ...
  refl::GlobalDef().def(
      "relax.frontend.nn.llm.kv_cache.attention_sequence_prefill",
      [](int64_t h_kv, int64_t h_q, int64_t d_qk, int64_t d_v, ffi::String dtype,
        ...
        // causal and sm_scale are accepted for API compatibility; the kernel
        // currently implements non-causal attention with unit scale.
