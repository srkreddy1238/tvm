...
tir::PrimFunc AttentionPrefillRagged(int64_t h_kv, int64_t h_q, int64_t d_qk, int64_t d_v,
  ...
  tir::Buffer q_rope_pos_buf = tir::Buffer(
      tir::decl_buffer({qo_len}, DataType::Int(32), "q_rope_position")->data, DataType::Int(32),
      {qo_len}, {}, q_rope_pos_eo, "q_rope_position", 0, 0, tir::kDefault);
  ...
  // K_smem: (tile_z, d_qk) — schedule's transform_layout will transpose to (d_qk, tile_z)
  ...
  auto causal_mask = [&](PrimExpr row, PrimExpr col, PrimExpr kv_len_e, PrimExpr qo_len_e) {
    return tvm::if_then_else(causal > I32(0), col < kv_len_e - qo_len_e + row + I32(1),
                             col < kv_len_e);
  };
  ...
  PrimExpr kv_chunk_expr =
      tir::BufferLoad(kv_indptr_buf, {b_idx + I32(1)}) - tir::BufferLoad(kv_indptr_buf, {b_idx});
      ...
      tir::For(
          ...
          tir::SBlockRealize(
              ...
              tir::SBlock(
                  ...
                  {}, {}, "Q_load",
                  tir::LetStmt(
                      cur_L_ql, q_indptr_val + floordiv(LH_start + vi_ql, I32(group_size)),
                      tir::LetStmt(
                          cur_H_qo_ql,
                          ...
                          tir::IfThenElse(
                              ...
                              tir::BufferStore(
                                  Q_smem,
                                  BuildPrefillRopeExpr(q_buf, {cur_L_ql, cur_H_qo_ql}, vj_ql, d_qk,
  ...
  // K_load — stored non-transposed: K_smem[i, j] = k[L_kv_base+cur_L, by, j]
  // schedule's transform_layout will transpose to K_smem[j, i]
                      ...
                      tir::IfThenElse(
                          ...
                          tir::BufferStore(
                              ...
                              BuildPrefillRopeExpr(
                                  k_buf, {L_kv_base + cur_L_kl, by}, vj_kl, d_qk,
                                  tir::BufferLoad(k_rope_pos_buf, {b_idx}) + cur_L_kl, rope_scale,
                                  rope_theta, rotary_mode, dtype, rope_scaling),
                              ffi::Array<PrimExpr>{vi_kl, vj_kl}),
                          tir::BufferStore(K_smem, zero_val, ffi::Array<PrimExpr>{vi_kl, vj_kl}))),
                  ...
                  tir::LetStmt(
                      ...
                      tir::IfThenElse(
                          ...
                          tir::BufferStore(
                              V_smem,
                              tir::BufferLoad(
                                  v_buf, ffi::Array<PrimExpr>{L_kv_base + cur_L_vl, by, vj_vl}),
                              ffi::Array<PrimExpr>{vi_vl, vj_vl}),
                          tir::BufferStore(V_smem, zero_val,
                                           ffi::Array<PrimExpr>{vi_vl, vj_vl})))))));

  // S_gemm
  // NOTE: Before transform_layout, K_smem has shape (tile_z, d_qk).
  // transform_layout applies (i,j)->(j,i) to K_smem, so all K_smem accesses
  // must use [j, k] order here (pre-transform). After transform_layout they
  // become [k, j] (post-transform) as seen in the expected IR.
  ...
  // Pre-transform: K_smem[j, k] — transform_layout will convert to K_smem[k, j]
  PrimExpr k_elem =
  ...
  // Before transform_layout, K_smem has shape (tile_z, d_qk).
  // transform_layout will apply (i,j)->(j,i) to the reads annotation,
  // converting K_smem[0:tile_z, 0:d_qk] -> K_smem[0:d_qk, 0:tile_z] in the final IR.
                  ...
                  tir::SBlock(
                      ...
                      tir::SeqStmt(
                          {tir::BufferStore(m_prev_buf, tir::BufferLoad(m_smem, {row_u1}), {i_u1}),
                           ...
                           tir::LetStmt(
                               row__u1,
                               floordiv(LH_start + row_u1, I32(group_size)),
                               tir::SeqStmt(
      ...
      tir::SBlock(
          ...
          tir::For(
              ...
              tir::IfThenElse(
                  row_u2 < I32(tile_x),
                  tir::LetStmt(
                      row__u2, floordiv(LH_start + row_u2, I32(group_size)),
                      tir::IfThenElse(
  ...
  auto o_store_block = tir::SBlock(
      ...
      tir::LetStmt(
          cur_L_os,
          tir::BufferLoad(q_indptr_buf, {b_idx}) + floordiv(LH_start + vi_os, I32(group_size)),
          tir::LetStmt(
  ...
  auto lse_store_block = tir::SBlock(
      ...
      tir::LetStmt(
          cur_L_lse,
          tir::BufferLoad(q_indptr_buf, {b_idx}) + floordiv(LH_start + vi_lse, I32(group_size)),
          tir::LetStmt(
  ...
  Stmt tile_body = tir::LetStmt(
      ...
      tir::LetStmt(
          ...
          tir::LetStmt(LH_start, tir::BufferLoad(tile_id_buf, {I32(0)}) * I32(tile_x),
                       tir::SeqStmt({st0(kv_chunk_buf, kv_chunk_expr), Sync(), init_md, o_init,
                                     Sync(), q_load, Sync(), kv_loop, o_store, lse_store,
  ...
  IterVar bx_iv(Range(nullptr), tir::Var("iter", DataType::Int(32)), tir::kThreadIndex,
                "blockIdx.x");
  IterVar by_iv(Range(nullptr), tir::Var("iter", DataType::Int(32)), tir::kThreadIndex,
                "blockIdx.y");
  IterVar ty_iv(Range(nullptr), tir::Var("iter", DataType::Int(32)), tir::kThreadIndex,
                "threadIdx.y");
  IterVar tx_iv(Range(nullptr), tir::Var("iter", DataType::Int(32)), tir::kThreadIndex,
                "threadIdx.x");

  // Spatial iter_vars for the "attn" sblock (mirrors T.axis.remap("SSSS", ...))
  IterVar attn_bx_iv(Range::FromMinExtent(I32(0), I32(NUM_BLKS)), bx, tir::kDataPar, "");
  IterVar attn_by_iv(Range::FromMinExtent(I32(0), I32(h_kv)), by, tir::kDataPar, "");
  IterVar attn_ty_iv(Range::FromMinExtent(I32(0), I32(num_warps)), ty, tir::kDataPar, "");
  IterVar attn_tx_iv(Range::FromMinExtent(I32(0), I32(bdx)), tx, tir::kDataPar, "");
  ffi::Array<tir::IterVar> attn_iter_vars = {attn_bx_iv, attn_by_iv, attn_ty_iv, attn_tx_iv};
  ffi::Array<PrimExpr> attn_iter_values = {lbx, lby, lty, ltx};
              ...
              tir::For(lby, I32(0), I32(h_kv), tir::ForKind::kThreadBinding,
                       tir::For(lty, I32(0), I32(num_warps), tir::ForKind::kThreadBinding,
                                tir::For(ltx, I32(0), I32(bdx), tir::ForKind::kThreadBinding,
                                         tir::SBlockRealize(
                                             attn_iter_values, tir::const_true(),
                                             tir::SBlock(attn_iter_vars, {}, {}, "attn",
  ...
  ffi::Array<tir::Var> params = {h_q_h,          h_q_indptr_h,   h_k_h,      h_v_h,   h_kv_indptr_h,
                                 h_q_rope_pos_h, h_k_rope_pos_h, h_output_h, h_lse_h, causal,
                                 rotary_mode,    rope_scale,     rope_theta, sm_scale};

  tir::PrimFunc fn(params, body, VoidType(), buf_map);
  fn = WithAttr(fn, "global_symbol", ffi::Any(ffi::String("batch_prefill_ragged_kv")));
...
TVM_FFI_STATIC_INIT_BLOCK() {
  ...
  refl::GlobalDef().def(
      ...
      [](int64_t h_kv, int64_t h_q, int64_t d_qk, int64_t d_v, ffi::String dtype,
        ...
        return AttentionPrefillRagged(h_kv, h_q, d_qk, d_v, std::string(dtype), rope_scaling,
                                      target);
      });
}
