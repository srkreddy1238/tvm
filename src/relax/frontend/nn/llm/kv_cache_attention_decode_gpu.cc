...
tir::PrimFunc AttentionDecode(int64_t num_kv_heads, int64_t num_qo_heads, int64_t head_dim,
                              ...
                              int64_t page_size) {
  const DecodeGpuConfig cfg = ComputeDecodeGpuConfig(
      num_kv_heads, num_qo_heads, head_dim, qkv_dtype, sliding_window, target);
  ...
  tir::Buffer pages_buf = tir::Buffer(
      tir::decl_buffer({max_num_pages, I32(2), I32(H_kv), I32(page_size), I32(D)}, dt, "pages")->data,
  ...
  tir::Buffer page_indptr_buf = tir::Buffer(
      ...
      DataType::Int(32), {B_var + I32(1)}, {}, page_indptr_eo, "page_table_indptr", 0, 0, tir::kDefault);
  ...
  tir::Buffer k_rope_pos_offset_buf = tir::Buffer(
      ...
      DataType::Int(32), {B_var}, {}, k_rope_pos_offset_eo, "k_rope_pos_offset", 0, 0, tir::kDefault);
  ...
  if (sliding_window) {
    ...
    last_page_len_expr = (num_pages - I32(1)) * I32(page_size) +
                         tir::BufferLoad(length_info_buf, {batch_idx});
  }
  ...
  Stmt init_states = tir::SeqStmt({
      st0(st_m_buf, F32(-50000.0)), st0(st_d_buf, F32(1.0)),
  ...
  PrimExpr q_rope_expr = BuildDecodeRopeExpr(Q_buf, {bx, h_qo_idx}, d_q, D, q_pos,
                                              rope_scale, rope_theta, rotary_mode, qkv_dtype, rope_scaling);
  ...
  Stmt kv_load_body = BuildDecodeKVLoadBody(
      cfg, pages_buf, page_values_buf, length_info_buf, k_rope_pos_offset_buf,
      K_smem, V_smem, kv_chunk_len, cur_begin, batch_idx, by_var, tx,
      tile_start_s_var, tile_start_g_var, j_kv, rope_scale, rope_theta, rotary_mode, rope_scaling);
  Stmt kv_load_sblock = tir::SBlockRealize({}, tir::const_true(),
      tir::SBlock({}, {}, {}, "KV_load", kv_load_body));
  Stmt j_kv_loop = tir::For(j_kv, I32(0), I32(tile), tir::ForKind::kSerial, kv_load_sblock);

  Stmt sync_shared = tir::Evaluate(tir::Call(DataType::Int(32),
      tir::builtin::tvm_storage_sync(), ffi::Array<PrimExpr>{tir::StringImm("shared")}));
  ...
  PrimExpr qk_elem = is_f16
      ? CastTo(ldi(Q_local, vec_qk), "float32") * CastTo(k_smem_elem, "float32") * sm_scale * F32(log2e)
  ...
  tir::CommReducer add_reducer(ffi::Array<Var>{x0}, ffi::Array<Var>{y0},
                              ffi::Array<PrimExpr>{x0 + y0},
                              ffi::Array<PrimExpr>{F32(0.0)});
  Stmt allreduce_sblock = tir::SBlockRealize({}, tir::const_true(),
      tir::SBlock(
          {},
          {tir::BufferRegion(S_reduce_local, {Range::FromMinExtent(I32(0), I32(1))})},
          {tir::BufferRegion(t0, {Range::FromMinExtent(I32(0), I32(1))})},
          "block_cross_thread",
          tir::AttrStmt(add_reducer, "reduce_scope",
              tvm::reinterpret(DataType::Handle(), IntImm(DataType::UInt(64), 0)),
              tir::Evaluate(tir::Call(DataType::Handle(), tir::builtin::tvm_thread_allreduce(),
                  ffi::Array<PrimExpr>{(PrimExpr)IntImm(DataType::UInt(32), 1), ld0(S_reduce_local),
                                       (PrimExpr)tir::const_true(), tir::BufferLoad(t0, {I32(0)}), (PrimExpr)tx})))));

  PrimExpr s_cond = (iterator * I32(bdz) + tz) * I32(bdy * tile) + j_qk < ld0(kv_chunk_len);
  Stmt s_update = tir::SeqStmt({
      sti(S_local, j_qk, F32(-50000.0)),
      tir::IfThenElse(s_cond, sti(S_local, j_qk, ld0(t0))),
      st0(st_m_buf, tvm::max(ld0(st_m_buf), ldi(S_local, j_qk)))});
  Stmt j_qk_body = tir::SeqStmt({qk_vec_loop, s_reduce_init, s_reduce_loop, allreduce_sblock, s_update});
  ...
  Stmt s_exp_loop = tir::For(j_s, I32(0), I32(bdy * tile), tir::ForKind::kSerial,
  ...
  Stmt o_scale_loop = tir::For(vec_oscale, I32(0), I32(VEC_SIZE), tir::ForKind::kVectorized,
  ...
  PrimExpr v_smem_elem = tir::BufferLoad(V_smem, {tz * I32(bdy * tile) + j_v, tx * I32(VEC_SIZE) + vec_vl});
  PrimExpr o_acc = is_f16
      ? ldi(O_local_buf, vec_oa) + CastTo(ldi(V_local, vec_oa), "float32") * ldi(S_local, j_v)
  ...
  Stmt softmax_update = tir::LetStmt(o_scale_var, o_scale_expr,
      tir::SeqStmt({st0(st_d_buf, ld0(st_d_buf) * o_scale_var), s_exp_loop, o_scale_loop, j_v_loop}));

  Stmt iter_body = tir::LetStmt(tile_start_s_var, tile_start_s_expr,
  ...
  Stmt allreduce_block = BuildDecodeAllreduceBlock(
      cfg, O_allreduce, md_allreduce, O_local_buf, st_m_buf, st_d_buf,
      m_prev_buf, d_prev_buf, other_m_buf, other_d_buf, other_o_buf,
      exp_mprev_buf, exp_otherm_buf, tz, ty, tx);

  tir::Var vec_norm("vec", DataType::Int(32));
  Stmt normalize = tir::For(vec_norm, I32(0), I32(VEC_SIZE), tir::ForKind::kVectorized,
      sti(O_local_buf, vec_norm, tvm::div(ldi(O_local_buf, vec_norm), ld0(st_d_buf))));
  tir::Var vec_out("vec", DataType::Int(32));
  PrimExpr out_val = is_f16 ? CastTo(ldi(O_local_buf, vec_out), qkv_dtype) : ldi(O_local_buf, vec_out);
  Stmt store_output = tir::For(vec_out, I32(0), I32(VEC_SIZE), tir::ForKind::kVectorized,
      tir::BufferStore(output_buf, out_val, {batch_idx, h_qo_idx, tx * I32(VEC_SIZE) + vec_out}));
  Stmt store_lse = tir::BufferStore(lse_buf, ld0(st_m_buf) + tvm::log2(ld0(st_d_buf)),
                                     {batch_idx, h_qo_idx});

  ffi::Array<tir::Buffer> alloc_bufs = {
      Q_local, kv_chunk_len, K_smem, V_smem, O_allreduce, md_allreduce,
      S_reduce_local, t0, S_local, QK_local, V_local,
      m_prev_buf, d_prev_buf, other_m_buf, other_d_buf,
      exp_mprev_buf, exp_otherm_buf, other_o_buf,
      st_m_buf, st_d_buf, O_local_buf};

  Stmt main_body;
  if (bdz > 1) {
    main_body = tir::SeqStmt({init_states, q_load, iter_loop, allreduce_block,
                               normalize, store_output, store_lse});
  } else {
    main_body = tir::SeqStmt({init_states, q_load, iter_loop, normalize, store_output, store_lse});
  }

  Stmt sblock_body = tir::LetStmt(by_var, floormod(fused_by_bz, I32(H_kv)),
      tir::LetStmt(bz_var, floordiv(fused_by_bz, I32(H_kv)),
          tir::LetStmt(batch_idx, bx,
              tir::LetStmt(cur_begin, tir::BufferLoad(page_indptr_buf, {batch_idx}),
  ...
  Stmt sblock = tir::SBlockRealize({}, tir::const_true(),
      tir::SBlock({}, {}, {}, "attn", sblock_body, std::nullopt, alloc_bufs,
                  {}, attn_annotations));
  ...
  body = tir::For(fused_by_bz, I32(0), I32(H_kv * gdz), tir::ForKind::kThreadBinding, body, fused_iv);
  ...
  body = tir::SBlockRealize({}, tir::const_true(),
      tir::SBlock({}, {}, {}, "root", body));

  ffi::Map<tir::Var, tir::Buffer> buf_map;
  buf_map.Set(h_Q, Q_buf); buf_map.Set(h_pages, pages_buf);
  buf_map.Set(h_page_indptr, page_indptr_buf); buf_map.Set(h_page_values, page_values_buf);
  buf_map.Set(h_length_info, length_info_buf); buf_map.Set(h_k_rope_pos_offset, k_rope_pos_offset_buf);
  buf_map.Set(h_q_rope_position, q_rope_position_buf); buf_map.Set(h_output, output_buf);
  buf_map.Set(h_lse, lse_buf);

  ffi::Array<tir::Var> params = {h_Q, h_pages, h_page_indptr, h_page_values, h_length_info,
      h_k_rope_pos_offset, h_q_rope_position, h_output, h_lse,
      rotary_mode, rope_scale, rope_theta, sm_scale};
