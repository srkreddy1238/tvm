...
/*!
 * \file src/relax/frontend/nn/llm/kv_cache_attention_prefill_mla_gpu.cc
 * \brief GPU TIR kernel for MLA (Multi-head Latent Attention) prefill.
 *        C++ port of _attention_prefill_mla() in kv_cache.py.
 *
 * MLA differences from standard paged prefill:
 *   - No separate K/V pages: a single KV_smem holds the compressed KV.
 *   - Pages shape: (max_num_pages, page_size, d_qk+d_v) — no head dimension.
 *   - Q shape: (total_len, num_heads, d_qk+d_v).
 *   - Output shape: (total_len, num_heads, d_v).
 *   - tile_x covers (seq_len * num_heads) jointly.
 *   - No RoPE (no rotary_mode / rope_scale / rope_theta parameters).
...
tir::PrimFunc AttentionPrefillMLA(int64_t num_heads, int64_t v_head_dim,
  ...
  // MLA uses a special config: tile_x covers (seq_len * num_heads)
  // We compute config with h_kv=1, h_q=num_heads, d=d_qk
  const PrefillKernelConfig cfg = ComputePrefillKernelConfig(1, num_heads, d_qk, dtype, target);
  const int64_t NUM_BLKS = cfg.NUM_BLKS;
  const int64_t bdx = cfg.bdx, num_warps = cfg.num_warps;
  // MLA tile_x = num_heads * (base_tile / num_heads) rounded up
  // Python: tile_x = num_heads * ceil(base_tile / num_heads)
  int64_t base_tile_x = cfg.tile_x;
  int64_t tile_x = num_heads * static_cast<int64_t>(std::ceil((double)base_tile_x / num_heads));
  int64_t tile_z = cfg.tile_z;  // KV tile size
  ...
  // ---- buffers ----
  // Q: (total_len, num_heads, d_qk)
  tir::Buffer q_buf = tir::decl_buffer({total_len, I32(num_heads), I32(d_qk)}, dt, "q");
  ...
  // ---- thread index vars ----
  // MLA: no blockIdx.y (no head dimension in grid)
  tir::Var lbx("lbx", DataType::Int(32));
