...
tir::PrimFunc AttentionPrefillRaggedCpu(int64_t h_kv, int64_t h_q, int64_t d_qk, int64_t d_v,
  ...
  PrimExpr q_elem_s = BuildPrefillRopeExpr(
      q_buf, {q_base + q_pos, h}, d_s, d_qk, tir::BufferLoad(q_rope_pos_buf, {q_base + q_pos}),
      rope_scale, rope_theta, rotary_mode, dtype, rope_scaling);
  PrimExpr k_elem_s = BuildPrefillRopeExpr(
      k_buf, {kv_base + kv_pos, h_kv_var}, d_s, d_qk, tir::BufferLoad(k_rope_pos_buf, {b}) + kv_pos,
      rope_scale, rope_theta, rotary_mode, dtype, rope_scaling);
  ...
  Stmt s_accum =
      tir::For(d_s, I32(0), I32(d_qk), tir::ForKind::kSerial,
               tir::BufferStore(
                   S_buf, tir::BufferLoad(S_buf, {kv_pos}) + q_f32 * k_f32 * sm_scale * F32(log2e),
                   {kv_pos}));
  Stmt s_mask = tir::IfThenElse(logical_not(causal_mask(q_pos, kv_pos, kv_chunk, q_len)),
  ...
  Stmt m_loop = tir::For(
      kv_pos2, I32(0), kv_chunk, tir::ForKind::kSerial,
      tir::BufferStore(
          m_buf, tvm::max(tir::BufferLoad(m_buf, {I32(0)}), tir::BufferLoad(S_buf, {kv_pos2})),
          {I32(0)}));
  Stmt d_init = tir::BufferStore(d_buf, F32(0.0), {I32(0)});
  Stmt softmax_loop = tir::For(
      kv_pos3, I32(0), kv_chunk, tir::ForKind::kSerial,
      tir::SeqStmt({tir::BufferStore(S_buf,
                                     tvm::exp2(tir::BufferLoad(S_buf, {kv_pos3}) -
                                               tir::BufferLoad(m_buf, {I32(0)})),
                                     {kv_pos3}),
                    tir::BufferStore(
                        d_buf, tir::BufferLoad(d_buf, {I32(0)}) + tir::BufferLoad(S_buf, {kv_pos3}),
                        {I32(0)})}));
  ...
  PrimExpr v_elem =
      is_f16
          ? CastTo(tir::BufferLoad(v_buf, ffi::Array<PrimExpr>{kv_base + kv_pos5, h_kv_var, d_o2}),
                   "float32")
  ...
  Stmt store_output = tir::For(
      d_out, I32(0), I32(d_v), tir::ForKind::kSerial,
      tir::BufferStore(
          output_buf,
          is_f16
              ? CastTo(tvm::div(tir::BufferLoad(O_buf, {d_out}), tir::BufferLoad(d_buf, {I32(0)})),
                       dtype)
              : tvm::div(tir::BufferLoad(O_buf, {d_out}), tir::BufferLoad(d_buf, {I32(0)})),
          ffi::Array<PrimExpr>{q_base + q_pos, h, d_out}));
  Stmt store_lse = tir::BufferStore(
      lse_buf, tir::BufferLoad(m_buf, {I32(0)}) + tvm::log2(tir::BufferLoad(d_buf, {I32(0)})),
      ffi::Array<PrimExpr>{q_base + q_pos, h});
  ...
  // ---- batch loop ----
  Stmt batch_body = tir::LetStmt(
      q_base, tir::BufferLoad(q_indptr_buf, {b}),
  ...
  Stmt body = tir::SBlockRealize(
      ...
      tir::SBlock({}, {}, {}, "root",
                  tir::SBlockRealize({}, tir::const_true(),
                                     tir::SBlock({}, {}, {}, "attn", batch_loop, std::nullopt,
                                                 alloc_bufs, {}, attn_ann))));
...
TVM_FFI_STATIC_INIT_BLOCK() {
  ...
  refl::GlobalDef().def("relax.frontend.nn.llm.kv_cache.attention_prefill_ragged_cpu",
                        [](int64_t h_kv, int64_t h_q, int64_t d_qk, int64_t d_v, ffi::String dtype,
                           ffi::Map<ffi::String, ffi::Any> rope_scaling) -> tir::PrimFunc {
                          return AttentionPrefillRaggedCpu(h_kv, h_q, d_qk, d_v, std::string(dtype),
                                                           rope_scaling);
                        });
}
