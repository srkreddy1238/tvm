...
inline bool GetStoreRule(ffi::Array<PrimExpr>* index_rule, ffi::Array<PrimExpr>* shape_rule,
  ...
  for (size_t i = 0; i < src_layout.ndim(); i++) {
    auto factor = src_layout[i]->dom->extent;
    auto src_unpacked_axes = Layout::UnpackIterVar(src_layout[i]);
    ...
    if (src_unpacked_axes.size() == 1 && LayoutAxis::Get(src_unpacked_axes[0]).IsPrimal()) {
      ...
      if (offset == -1)
        norm_indexes[prim_axis.name()[0] - 'A'] =
            norm_indexes[prim_axis.name()[0] - 'A'] + src_layout[i];
      else
        norm_indexes[prim_axis.name()[0] - 'A'] = norm_indexes[prim_axis.name()[0] - 'A'] +
                                                  src_layout[i] * src_layout.FactorOf(prim_axis);
      ...
      for (size_t j = 0; j < src_unpacked_axes.size(); j++) {
        ...
        const IterVar& store_axis_impl = src_unpacked_axes[j];
        const LayoutAxis& sub_axis =
            LayoutAxis::Get(store_axis_impl).ToSubordinate(); /* Not Needed */
        const LayoutAxis& prim_axis = LayoutAxis::Get(store_axis_impl).ToPrimal();

        PrimExpr factor_ij = indexdiv(src_layout[i], index_divs[j]);
        ...
        for (size_t k = i; k < src_layout.ndim(); k++) {
          ...
          auto inter_unpacked_axes = Layout::UnpackIterVar(src_layout[k]);
  ...
  for (size_t i = 0; i < dst_layout.ndim(); i++) {
    const auto dst_unpacked_axes = Layout::UnpackIterVar(dst_layout[i]);
    ...
    if (dst_unpacked_axes.size() == 1 && LayoutAxis::Get(dst_unpacked_axes[0]).IsPrimal()) {
      ...
      for (size_t j = 0; j < dst_unpacked_axes.size(); j++) {
        ...
        for (size_t k = i; k < dst_layout.ndim(); k++) {
          ...
          const auto inter_unpacked_axes = Layout::UnpackIterVar(dst_layout[k]);
          for (; l < inter_unpacked_axes.size(); l++) {
