 ...
 */
/*!
 * \file src/s_tir/backend/adreno/pipeline.cc
 * S-TIR Adreno GPU pipeline
 */
...
using tvm::s_tir::backend::GenericTIRPipeline;

class AdrenoTIRPipeline : GenericTIRPipeline {
 public:
  explicit AdrenoTIRPipeline(const Target& target) : GenericTIRPipeline(target) {}

  virtual IRModule Base(IRModule mod) {
    auto pass_ctx = tvm::transform::PassContext::Current();
    mod = tvm::s_tir::backend::adreno::transform::TextureFlatten()(mod);
    ...
    mod = tvm::s_tir::backend::adreno::transform::InjectTextureAlloc()(mod);
  ...
  IRModule Host(IRModule mod) { return GenericTIRPipeline::Host(mod); }

  IRModule Device(IRModule mod) { return GenericTIRPipeline::Device(mod); }
};

namespace pipeline {
TVM_S_TIR_BACKEND_PIPELINE(adreno, AdrenoTIRPipeline);
