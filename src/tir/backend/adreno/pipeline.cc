...
using tvm::tir::backend::GenericTIRPipeline;
...
class AdrenoTIRPipeline : GenericTIRPipeline {
 public:
  ...
  virtual IRModule Base(IRModule& mod) {
    ...
    mod = tir::backend::adreno::transform::TextureFlatten()(mod);
    ...
    mod = tir::backend::adreno::transform::InjectTextureAlloc()(mod);
  ...
  virtual IRModule Host(IRModule& mod) { return GenericTIRPipeline::Host(mod); }

  virtual IRModule Device(IRModule& mod) { return GenericTIRPipeline::Device(mod); }
};

namespace pipeline {
TVM_TIR_BACKEND_PIPELINE(adreno, AdrenoTIRPipeline);
