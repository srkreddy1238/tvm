  ...
  for (size_t i = 0; i < attrs_.shape.size(); ++i) {
    ...
    if (!details::Is2DStorage(storage_scope)) {
      ...
      const auto& cit = std::find_if(
          ...
          [&dev_type](const Device& d) { return dev_type == static_cast<int>(d.device_type); });
      Device dev = cit == devices_.end() ? devices_[0] : *cit;
      auto device_api = DeviceAPI::Get(dev);
      tvm::runtime::TVMRetValue ret;
      device_api->GetAttr(dev, tvm::runtime::DeviceAttrKind::kImagePitchAlignment, &ret);
      int image_pitch_alignment = ret;
      int64_t alloc_size = runtime::GetTextureMemorySize<std::vector<int64_t>>(
          attrs_.shape[i], t.bits, t.lanes, storage_scope, image_pitch_alignment);
