...
size_t VulkanDeviceAPI::GetDataSize(const DLTensor& arr, Optional<String> mem_scope) {
...
void* VulkanDeviceAPI::AllocDataSpaceView(Device dev, void* data, ShapeTuple shape,
...
// Register Vulkan texture allocation functions
TVM_REGISTER_GLOBAL("device_api.vulkan.alloc_nd").set_body([](TVMArgs args, TVMRetValue* rv) {
  ...
  *rv = VulkanDeviceAPI::Global()->AllocDataSpace(
      dev, static_cast<size_t>(width), static_cast<size_t>(height), static_cast<size_t>(depth),
      type_hint, Optional<String>("global.texture"));
});

TVM_REGISTER_GLOBAL("device_api.vulkan.free_nd").set_body([](TVMArgs args, TVMRetValue* rv) {
  int32_t device_type = args[0];
  int32_t device_id = args[1];
...
TVM_REGISTER_OBJECT_TYPE(VulkanTimerNode);

TVM_REGISTER_GLOBAL("profiling.timer.vulkan").set_body_typed([](Device dev) {
  return Timer(make_object<VulkanTimerNode>(dev));
});
