...
class OpenCLWrappedFunc {
 public:
  ...
  void dump_trace(const ThreadWorkLoad& wl, int work_dim, const TVMArgs& args) const {
    ...
    for (cl_uint i = 0; i < arg_size_.size(); ++i) {
      ...
      if (args.type_codes[i] == DLDataTypeCode::kDLOpaqueHandle) {
        cl::BufferDescriptor* desc = static_cast<cl::BufferDescriptor*>(args.values[i].v_handle);
