...
def schedule_inline_blocks(sch: tir.Schedule, blocks: List[tir.schedule.BlockRV] = None):
    """Inlines Elementwise/Broadcast Blocks. Selectively Schedules Blocks if patterns are supplied."""
...
def schedule_default(sch, blocks: List[tir.schedule.BlockRV] = None):
    """Applies Default Spatial Schedules to the Given Blocks. Selectively Schedules Blocks if patterns are supplied."""
    from .fallback import Fallback
...
def schedule_storage_annotate(sch: tir.Schedule, func=get_texture_storage):
    """Annotates Intermediate storage buffers. Selectively Schedules Blocks if patterns are supplied."""
